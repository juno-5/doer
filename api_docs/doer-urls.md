...
the Doer web app uses for various types of views.
...
for the Doer server (In these examples,
`https://doer.example.com/`).

## Message feed views

Most links in Doer are to message feed views, and for that reason
...
`https://doer.example.com/#narrow/is/starred/sender/17/-channel/14`
...
representations of Doer-internal links that will be translated into
...
- `42-channel-name`. The ID of the channel, with a human-readable hint
  of the channel name. Clients generating Doer URLs are recommended
...
These two formats allow Doer URLs to stably refer to a specific
...
## doer:// links for mobile login

Doer's single-sign on login process for the mobile app ends with a
redirect to `doer://login` with the following query parameters:
...
- `otp_encrypted_api_key`: The API key for the client, encrypted using
  ...
  login attempt.
- `realm`: The full URL of the Doer organization.
- `user_id`: The Doer user ID for the authenticated account.

**Changes**: The `user_id` field was added to the set of included
query parameters in Doer 5.0 (feature level 128).
