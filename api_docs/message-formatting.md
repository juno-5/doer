...
Doer supports an extended version of Markdown for messages, as well as
some HTML level special behavior. The Doer help center article on [message
formatting](/help/format-your-message-using-markdown) is the primary
documentation for Doer's markup features. This article is currently a
...
**Changes**: As of Doer 4.0 (feature level 33), [code blocks][help-code]
...
**Changes**: In Doer 12.0 (feature level 451), invalid timestamp formats
...
**Changes**: In Doer 3.0 (feature level 8), added [global time
mentions][help-global-time] to supported Markdown message formatting
features.

## Links to channels, topics, and messages

Doer's markup supports special readable Markdown syntax for [linking
to channels, topics, and
messages](/help/link-to-a-message-or-conversation). See also [Doer
URLs](/api/doer-urls)
...
<!-- Syntax: #**announce>Doer updates** -->
<a class="stream-topic" data-stream-id="9"
  href="/#narrow/channel/9-announce/topic/Doer.20updates/with/214">
 #announce &gt; Doer updates
</a>

<!-- Syntax: #**announce>Doer updates**
     Generated only if topic had no messages or the link was rendered
     before Doer 10.0 (feature level 347) -->
<a class="stream-topic" data-stream-id="9"
  href="/#narrow/channel/9-announce/topic/Doer.20updates">
 #announce &gt; Doer updates
</a>

<!-- Syntax: #**announce>Doer updates@214** -->
<a class="message-link"
  href="/#narrow/channel/9-announce/topic/Doer.20updates/near/214">
 #announce &gt; Doer updates @ 💬
</a>
```
...
**Changes**: In Doer 11.0 (feature level 400), the server switched
...
Before Doer 10.0 (feature level 347), the `with` field
...
Doer's [emoji][help-emoji] support includes standard Unicode emoji, a
built-in Doer custom emoji like `:doer:` and [custom realm
...
size. For example, Doer the web app scales large emoji to be 2x the
...
Note that Unicode emoji in messages that were originally sent on Doer
versions older than Doer 1.9.2 will not have the `role` or
...
<-- Built-in custom emoji like :doer: -->
<img alt=":doer:" class="emoji" title="doer"
  src="/static/generated/emoji/images/emoji/unicode/doer.png">
```

**Changes**: Large emoji are new in Doer 12.0 (feature level 436).

## Images

When a Doer message is sent linking to an uploaded image, Doer will
...
**In Doer 10.0** (feature level 336), added
...
Previously, with the exception of Doer servers that used the beta
Thumbor-based implementation years ago, all image previews in Doer
...
When a Doer message is sent linking to an uploaded video, Doer may
...
audio `Content-Type`, Doer will generate an HTML5 `<audio>` player
...
If the Doer server has rewritten the URL of the audio file, it will
provide the URL in a `data-original-url` parameter. The Doer server
...
The Doer server does not validate whether uploaded files with an
audio `Content-Type` are actually playable.

**Changes**: New in Doer 11.0 (feature level 405).

## Mentions and silent mentions

Doer markup supports [mentioning](/help/mention-a-user-or-group)
...
In Doer 11.0 (feature level 395), the Doer server stopped generating
legacy Dropbox link previews. Dropbox links are now previewed just
like standard Doer image/link previews. However, some legacy Dropbox
...
In Doer 4.0 (feature level 24), the rarely used `!avatar()`
