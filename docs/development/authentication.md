...
between Doer, an external service, and the user's browser. Because
...
also serves as `/etc/zulip/doer-secrets.conf`).

Below, we document the procedure for each of the major authentication
methods supported by Doer.

### Email and password

Doer's default EmailAuthBackend authenticates users by verifying
...
- All of our authentication flows in the development environment have
  ...
  which shows all emails that the Doer server has "sent" (emails are
...
be `http://auth.{your_droplet_name}.zulipdev.org:9991/complete/saml/`
  - `http://localhost:9991` for the "Audience URI (SP Entity ID)". If you are using a dev droplet, this will be
    `http://doer.{your_droplet_name}.zulipdev.org:9991/`
...
URL they post back to, which isn't supported directly by the Doer
development environment. If you run a
[remote Doer development server](remote.md), we have
instructions for
...
Before Doer 2.0, one of the more common classes of bug reports with
Doer's authentication was users having trouble getting [LDAP
...
popular [fakeldap](https://github.com/doer/fakeldap) library. And in
2018, we added convenient support for using fakeldap in the Doer
...
Doer uses [django-two-factor-auth][0] as a beta 2FA integration.

To enable 2FA, set `TWO_FACTOR_AUTHENTICATION_ENABLED` in settings to
`True`, then log in to Doer and add an OTP device from the settings
...
By default, Doer uses `autocomplete=off` for password fields where we
