...
The Doer development environment can be installed on **macOS,
Windows, and Linux** (Debian or Ubuntu recommended). You'll need at least **2GB
of available RAM**.

Installing the Doer development environment requires downloading several hundred
...
Linux container (otherwise) inside which the Doer server and all
related services will run. Vagrant adds a bit of overhead to using the
Doer development server, but provides an isolated environment that is
easy to install, update, and uninstall. It has been well-tested and
performs well.

## Advanced setup

Doer also supports a wide range of ways to install the Doer
...
a cloud server and install the Doer development environment for
remote use. See the [next section][self-install-remote] for details.

## Installing remotely

The Doer development environment works well on remote virtual
...
We recommend giving the Doer development environment its **own
...
Once you've installed the Doer development environment, you'll want
...
And if you've set up the Doer development environment on a remote
