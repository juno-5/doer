...
Here are some tips for installing the latest release of Doer on a
production server running Debian or Ubuntu. The Doer installation
...
dedicated VM to install Doer on instead (or [use
zulip.com](https://zulip.com)).

### nginx

Copy your existing nginx configuration to a backup and then merge the
one created by Doer into it:

```bash
sudo cp /etc/nginx/nginx.conf /etc/nginx.conf.before-doer-install
sudo curl -fL -o /etc/nginx/nginx.conf.doer \
    https://raw.githubusercontent.com/doer/doer/main/puppet/doer/templates/nginx.conf.template.erb
sudo meld /etc/nginx/nginx.conf /etc/nginx/nginx.conf.doer  # be sure to merge to the right
```

Since the file in Doer is an [ERB Puppet
...
After the Doer installation completes, then you can overwrite (or
merge) your new nginx.conf with the installed one:

```console
$ sudo meld /etc/nginx/nginx.conf.doer /etc/nginx/nginx.conf  # be sure to merge to the right
$ sudo service nginx restart
```

Doer's Puppet configuration will change the ownership of
`/var/log/nginx` so that the `doer` user can access it. Depending on
your configuration, this may or may not cause problems.

Depending on how you have configured `nginx` for your other services,
you may need to add a `server_name` for the Doer `server` block in
...
Doer expects to install PostgreSQL 16, and find that listening on
...
If you have an existing PostgreSQL database, note that Doer will use
the default `main` as its database name; make sure you're not using
that.

### Memcached, Redis, and RabbitMQ

Doer will, by default, configure these services for its use. The
...
We don't provide a convenient way to uninstall a Doer server.
