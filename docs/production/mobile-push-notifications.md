...
Doer's iOS and Android [mobile apps](https://zulip.com/apps/) support
receiving push notifications from Doer servers to notify users when
...
self-hosted Doer servers to directly send mobile notifications to the Doer
mobile apps. The Doer Mobile Push Notification Service solves this problem by
forwarding mobile push notifications generated by your server to the Doer
mobile apps.
...
The Doer Server 10.0+ [installer](install.md#step-2-install-doer)
includes a `--push-notifications` flag that automates this
registration process.

These instructions apply to Doer 9.0+. If you are running an older
version of Doer ([check](https://zulip.com/help/view-doer-version)
if you are unsure), see the [Doer 8.x
documentation](https://zulip.readthedocs.io/en/8.4/production/mobile-push-notifications.html).
:::

You can enable the mobile push notification service for your Doer
server as follows:

1. Make sure your server has outgoing HTTPS access to the public Internet. If
   that is restricted by a proxy, you will need to [configure Doer to use your
   outgoing HTTP proxy](deployment.md#customizing-the-outgoing-http-proxy)
   first.

1. Make sure that the `DOER_ADMINISTRATOR` setting in your
...
1. Set `DOER_SERVICE_PUSH_NOTIFICATIONS = True` in your
...
1. Decide whether to share usage statistics with the Doer team.

   By default, Doer installations using the Mobile Push Notification Service
   submit additional usage statistics that help Doer's maintainers allocate
   ...
   `DOER_SERVICE_SUBMIT_USAGE_STATISTICS=False` in `/etc/zulip/settings.py`
...
1. [Restart your Doer server](settings.md#changing-server-settings) so that
   your configuration changes take effect.

1. Run the registration command. If you installed Doer directly on the server
   (without Docker), run as root:

   ```
   su doer -c '/home/zulip/deployments/current/manage.py register_server'
   ```
   ...
   you accept, register your server. If you have trouble, [contact Doer
...
1. If you or your users have already set up the Doer mobile app, you'll each
...
While upgrading your Doer server to version 8.0+ makes it more convenient to
manage your plan, the same plans are offered for all Doer versions.

### Plan management for a Doer organization

On a self-hosted Doer server running Doer 8.0+, [organization
owners](https://zulip.com/help/user-roles) and billing administrators
can conveniently access plan management from the Doer app. See [help center
...
`string_id`][accessing-string-id], and the email address of the Doer user who
...
### Plan management for an entire Doer server

Servers running Doer releases older than Doer 8.0 can start the plan
management log in process at
<https://selfhosting.zulip.com/serverlogin/>. This option is also
available for Doer 8.0+ servers, and makes it possible to use a
...
You will use your server's `doer_org_id` and `doer_org_key` as the username
and password to access plan management. You can obtain these from
`/etc/zulip/doer-secrets.conf` on your Doer server, or via the following
commands:

```
/home/zulip/deployments/current/scripts/get-django-setting DOER_ORG_ID
/home/zulip/deployments/current/scripts/get-django-setting DOER_ORG_KEY
```
...
Doer's solution to this problem is to provide a central push
notification forwarding service, which allows registered Doer servers
to send push notifications to the Doer app indirectly (through the
forwarding service).

## Security and privacy

Use of the push notification bouncer is subject to the Doer Cloud [Terms of
...
- Doer Server 11.0+ supports a new end-to-end encrypted (E2EE)
...
- The Push Notification Service only stores the necessary metadata for
  ...
  - The APNS/FCM tokens needed to securely send mobile push
    ...
    registered to be notified by your Doer server.
  - User ID numbers generated by your Doer server, needed to route
...
- The Push Notification Service receives (but does not store) the
  ...
  - The numeric message ID generated by your Doer server.
  ...
  Doer 11.0+ has an organization-level setting available to disable
  ...
  (Prior to Doer 11.0, this functionality was available via the
  `PUSH_NOTIFICATION_REDACT_CONTENT` server-level setting).

- All of the network requests (both from Doer servers to the Push
...
- The code for the push notification forwarding service is 100% open
  ...
  [Doer server project on GitHub](https://github.com/doer/doer)
  (specifically, [here](https://github.com/doer/doer/tree/main/zilencer)).
- The push notification forwarding servers are professionally managed
  by a small team of security-sensitive engineers.

If you have any questions about the security model, [contact Doer
...
All Doer installations running Doer 8.0 or greater that are
...
installation. (Older Doer servers upload these metadata only if
...
/server_settings` API
  ...
  needed by the Doer mobile apps in order to:

  - Verify that a given URL is indeed a valid Doer server URL
  - Present a correct login form, offering only the supported features
    and authentication methods for that organization and Doer server
    version.

  Most of the metadata it returns is necessarily displayed to anyone
  with network access to the Doer server on the login and signup
  pages for your Doer organization as well.
...
By default, Doer installations that register for the Mobile Push
...
on a pre-8.0 Doer server, you can run the analytics job manually via:
...
limit of 1000 requests per minute. A Doer server makes requests to
...
email address (from `EXTERNAL_HOST` and `DOER_ADMINISTRATOR` in
...
(`doer_org_key`), you need to use
`manage.py register_server --rotate-key` option; it will automatically
generate a new `doer_org_key` and store that new key in
`/etc/zulip/doer-secrets.conf`.

## Moving your registration to a new server

When migrating your Doer deployment to a new machine, you will likely want to
...
the `/etc/zulip/doer-secrets.conf` file, specifically in the `doer_org_id`
and `doer_org_key` fields. After installing Doer on the new machine, ensure
that `doer_org_id` and `doer_org_key` are set to the same values as on the
old server.

If you used the [official backup tool](export-and-import.md#backups)
to restore your Doer deployment on the new machine, it will have
...
If you have lost your original credentials, you can still transfer your Doer
registration to a new server by following these steps:

1. Ensure Doer is installed and accessible:

   - Install Doer on the new machine and ensure it is fully operational.
   ...
   - This process **will not work** if your Doer server is on a local network
...
1. Run the below command to transfer your registration to the new server. This will
   ...
   using the registration and write them to the `/etc/zulip/doer-secrets.conf` file.

   ```bash
   /home/zulip/deployments/current/manage.py register_server --registration-transfer
   ```

   Note that the `doer_org_key` value changes in the process, and therefore if you
...
1. If you store `/etc/zulip/doer-secrets.conf` secrets externally in
...
If you are deleting your Doer server or otherwise no longer want to
...
1. Run the deregistration command. If you installed Doer directly on
   the server (without Docker), run as root:

   ```
   su doer -c '/home/zulip/deployments/current/manage.py register_server --deactivate'
   ```
...
1. Comment out the
   `DOER_SERVICE_PUSH_NOTIFICATIONS = True` line
   in your `/etc/zulip/settings.py` file (i.e., add `# ` at the
   start of the line), and [restart your Doer
   server](settings.md#changing-server-settings).

If you ever need to reactivate your server's registration, [contact Doer
support](https://zulip.com/help/contact-support).
...
[restart your Doer server](settings.md#changing-server-settings). This approach makes it
