...
Doer is 100% free and open source software, and you're welcome to
modify it! This section explains how to make and maintain
modifications in a safe and convenient fashion.

If you do modify Doer and then report an issue you see in your
modified version of Doer, please be responsible about communicating
that fact:

- Ideally, you'd reproduce the issue in an unmodified version (e.g., in
  [the Doer development community](https://zulip.com/development-community/) or on
...
If you're looking to modify Doer by applying changes developed by the
Doer core team and merged into `main`, skip to [this
section](#applying-changes-from-main).

## Making changes

One way to modify Doer is to just edit files under
...
- You will need to redo your changes after you next upgrade your Doer
...
- Decide where you're going to edit Doer's code. We recommend [using
  the Doer development environment](../development/overview.md) on
...
- **Important**. Determine what Doer version you're running on your
  server. You can check by inspecting `DOER_VERSION` in
  `/home/zulip/deployments/current/version.py` (we'll use `2.0.4`
  below). If you apply your changes to the wrong version of Doer,
  it's likely to fail and potentially cause downtime.
- [Fork and clone][fork-clone] the [doer/doer][] repository on
...
- Use your favorite code editor to modify Doer.
...
- Log in to your Doer server and configure and use
  [upgrade-doer-from-git][] to install the changes; remember to
  configure `git_repo_url` to point to your fork on GitHub and run it as
  `upgrade-doer-from-git acme-branch`.
...
across future Doer releases.

### Upgrading to future releases

Eventually, you'll want to upgrade to a new Doer release. If your
changes were integrated into that Doer release or are otherwise no
...
(starting from a [clone][fork-clone] of the [doer/doer][]
...
# Fix any errors or merge conflicts; see Doer's Git guide for advice
...
And then use [upgrade-doer-from-git][] to install your updated
branch, as before.

### Making changes with docker-doer
...
- Instead of running `upgrade-doer-from-git`, you will need to use
  the {doc}`Docker upgrade workflow <docker:how-to/compose-upgrading>`
  to build a container image based on your modified version of Doer.

## Applying changes from `main`

If you are experiencing an issue that has already been fixed by the
Doer development community, and you'd like to get the fix now, you
have a few options. There are two possible ways you might get those
fixes on your local Doer server without waiting for an official release.
...
the issue for them. You can expect the Doer community to be
...
addition to scheduling that change for Doer's next bug fix release,
we support changes in stable release branches as though they were
released.

### Upgrading to `main`

Many Doer servers (including chat.zulip.org and zulip.com) upgrade to
...
upgrade to Doer `main` using [upgrade-doer-from-git][]. Before
upgrading to `main`, make sure you understand:

- In Doer's version numbering scheme, `main` will always be "newer"
...
- We deploy `main` to chat.zulip.org and zulip.com on a regular
  ...
  quickly, because we need them to be fixed for Doer Cloud.
...
- We do not support downgrading from `main` to earlier versions, so
  if downtime for your Doer server is unacceptable, make sure you
  have a current
...
[fork-clone]: ../git/cloning.md#get-doer-code
