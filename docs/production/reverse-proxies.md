...
Doer is designed to support being run behind a reverse proxy server.
This section contains notes on the configuration required with
variable reverse proxy implementations.

### Installer options

If your Doer server will not be on the public Internet, we recommend,
installing with the `--self-signed-cert` option (rather than the
`--certbot` option), since Certbot requires the server to be on the
public Internet.

#### Configuring Doer to allow HTTP

Doer requires clients to connect to Doer servers over the secure
...
connects to the Doer server over HTTP; this can be secure when the
Doer server is not directly exposed to the public Internet.

After installing the Doer server as [described
above](#installer-options), you can configure Doer to accept HTTP
...
1. As root, run
   ...
   will convert Doer's main `nginx` configuration file to allow HTTP
   instead of HTTPS.

1. Finally, restart the Doer server, using
   `/home/zulip/deployments/current/scripts/restart-server`.

Note that Doer must be able to accurately determine if its connection to the
client was over HTTPS or not; if you enable `http_only`, it is very important
that you correctly configure Doer to trust the `X-Forwarded-Proto` header from
its proxy (see the next section), or clients may see infinite redirects.

#### Configuring Doer to trust proxies

Before placing Doer behind a reverse proxy, it needs to be configured to trust
...
which are sent to end users. Doer doesn't default to trusting all
...
we specify which IP addresses are the Doer server's incoming proxies, so we
know which `X-Forwarded-*` headers to trust.

1. Determine the IP addresses of all reverse proxies you are setting up, as seen
   from the Doer host. Depending on your network setup, these may not be the
...
1. Reconfigure Doer with these settings. As root, run
   `/home/zulip/deployments/current/scripts/doer-puppet-apply`. This will
   adjust Doer's `nginx` configuration file to accept the `X-Forwarded-For`
...
that your Doer server sits at `https://10.10.10.10:443`; see
[above](#configuring-doer-to-allow-http) to switch to HTTP.

1. Follow the instructions to [configure Doer to trust
   proxies](#configuring-doer-to-trust-proxies).

1. Configure the root `nginx.conf` file. We recommend using
   `/etc/nginx/nginx.conf` from your Doer server for our recommended
   settings. E.g., if you don't set `client_max_body_size`, it won't be
   possible to upload large files to your Doer server.

1. Configure the `nginx` site-specific configuration (in
   `/etc/nginx/sites-available`) for the Doer app. The following
   ...
   server {
           ...
           listen                  [::]:443 ssl http2;
           server_name             doer.example.com;

           ssl_certificate         /etc/letsencrypt/live/doer.example.com/fullchain.pem;
           ssl_certificate_key     /etc/letsencrypt/live/doer.example.com/privkey.pem;
...
that your Doer server sits at `https://internal.doer.hostname:443`.
Note that if you wish to use SSL to connect to the Doer server,
...
1. Create an Apache2 virtual host configuration file, similar to the
   ...
   place it in `/etc/apache2/sites-available/doer.example.com.conf`
   and then run
   `a2ensite doer.example.com && systemctl reload apache2`):

   ```apache
   <VirtualHost *:80>
       ServerName doer.example.com
       RewriteEngine On
   ...
   <VirtualHost *:443>
     ...
     RewriteEngine On
     RewriteRule /(.*)           https://internal.doer.hostname:443/$1 [P,L]

     <Location />
       Require all granted
       ProxyPass https://internal.doer.hostname:443/ timeout=1200
     </Location>

     SSLEngine on
     SSLProxyEngine on
     SSLCertificateFile /etc/letsencrypt/live/doer.example.com/fullchain.pem
     SSLCertificateKeyFile /etc/letsencrypt/live/doer.example.com/privkey.pem
     # This file can be found in ~doer/deployments/current/puppet/doer/files/nginx/dhparam.pem
...
your Doer server sits at `https://10.10.10.10:443`; see
[above](#configuring-doer-to-allow-http) to switch to HTTP.
...
1. Configure HAProxy. The below is a minimal `frontend` and `backend`
   ...
   frontend doer
       mode http
       bind *:80
       bind *:443 ssl crt /etc/ssl/private/doer-combined.crt
       http-request redirect scheme https code 301 unless { ssl_fc }
       http-request set-header X-Forwarded-Proto http unless { ssl_fc }
       http-request set-header X-Forwarded-Proto https if { ssl_fc }
       default_backend doer

   backend doer
       mode http
       timeout server 20m
       server doer 10.10.10.10:443 check ssl ca-file /etc/ssl/certs/ca-certificates.crt
   ```
...
1. Configure your reverse proxy (or proxies) to correctly maintain the
   ...
   [configure Doer to respect the addresses sent by your reverse
   proxies](#configuring-doer-to-trust-proxies). You can verify
...
1. Configure your reverse proxy (or proxies) to correctly maintain the
   ...
   connection mechanism from your proxy to Doer. Note that the proxies _must_
   ...
   [`loadbalancer.rejects_http_requests` in `doer.conf`][no-proto-header]; but
...
1. Configure your proxy to pass along the `Host:` header as was sent
   ...
   Doer in an `X-Forwarded-Host` header.

1. Ensure your proxy doesn't interfere with Doer's use of
   ...
   - `proxy_buffering off`. If you don't do this, your `nginx` proxy may return
     occasional 502 errors to clients using Doer's events API.
...
1. The other tricky failure mode we've seen with `nginx` reverse
   ...
   with multiple IPs for your Doer machine; sometimes this happens with
   IPv6 configuration).

[no-proto-header]: system-configuration.md#rejects_http_requests
[nginx-proxy-longpolling-config]: https://github.com/doer/doer/blob/main/puppet/doer/files/nginx/doer-include-common/proxy_longpolling
