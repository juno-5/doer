...
This page offers practical information on how to secure your Doer server. For a
deeper understanding of the security model, take a look at Doer's [security
overview](https://zulip.com/security/).

Here are some best practices for keeping your Doer server secure:
...
4. [Become familiar with Doer's access management model.](#4-become-familiar-with-zulips-access-management-model)
...
6. [Understand Doer's rate-limiting system.](#6-understand-zulips-rate-limiting-system)
...
Anyone with root access to a Doer application server or Doer database server,
or with access to the `doer` user on a Doer application server, has _complete
control over the Doer installation_ and all of its data (so they can read
messages, modify history, etc.). This means that _only trusted individuals_
should have shell access to your Doer server.

## 2. Consider requiring authentication with single sign-on (SSO).

The preferred way to log in to Doer is using a single sign-on (SSO)
solution like Google authentication, LDAP, or similar, but Doer
also supports password authentication. See [the authentication
methods documentation](authentication-methods.md) for
details on Doer's available authentication methods.

## 3. Teach users how to protect their account.

Every Doer user has an API key, which can be used to do essentially everything
...
## 4. Become familiar with Doer's access management model.

Doer's help center offers a detailed overview of Doer's permissions management
...
- Doer supports user-uploaded files. Ideally they should be hosted
  from a separate domain from the main Doer server to protect against
  various same-domain attacks (e.g., doer-user-content.example.com).
  ...
  Doer server's filesystem, and the S3 backend, where the files are
  ...
  users accessing files uploaded in Doer (an authorized user would
  ...
  a Doer account that has received the uploaded file in question).

- Doer supports using the [go-camo][go-camo] image proxy to proxy content like
  inline image previews, that can be inserted into the Doer message feed by
  other users. This ensures that clients do not make requests to external
  servers to fetch images, improving privacy.

- By default, Doer will provide image previews inline in the body of
  messages when a message contains a link to an image. You can
  control this using the `INLINE_IMAGE_PREVIEW` setting.

- Doer may make outgoing HTTP connections to other servers in a
  number of cases:
  ...
  - BigBlueButton, Constructor Groups, and Zoom API requests
...
- Notably, these first 3 features give end users (limited) control to cause
  the Doer server to make HTTP requests on their behalf. Because of this,
  Doer routes all outgoing HTTP requests [through
  Smokescreen][smokescreen-setup] to ensure that Doer cannot be
...
## 6. Understand Doer's rate-limiting system.

Doer has built-in rate limiting of login attempts, all access to the
...
with an existing Doer account. Limiting the ability of users to
trigger such emails helps prevent bad actors from damaging the spam
reputation of a Doer server by sending confirmation emails to random
email addresses.

The default rate limiting rules for a Doer server will change as we improve
...
RATE_LIMITING_RULES` to verify your changes. You can then restart
  the Doer server with `scripts/restart-server` to have the new
