...
main configuration file used by system administrators to configure their Doer
server.

[Organization administrators][user-roles] can also [configure][realm-admin-docs]
many options for a Doer organization from the web or desktop app. See [system
and deployment configuration documentation](system-configuration.md) for advanced
configuration of the various services that make up a complete Doer installation
(`/etc/zulip/doer.conf`).
...
The Doer server self-documents more than a hundred settings in the
...
[settings-py-template]: https://github.com/doer/doer/blob/main/zproject/prod_settings_template.py

Important settings in `/etc/zulip/settings.py` include:

- The mandatory `EXTERNAL_HOST` and `DOER_ADMINISTRATOR` settings,
...
file on your Doer server, and restart the server with the following command:
...
support is available in the [Doer development community][chat-doer-org].
...
tour](https://static.zulipchat.com/static/navigation-tour-video/doer-10.mp4),
which shows where to find everything to get started with Doer.
...
video with information on how your organization uses Doer. A value of `None`
...
appears prominently in all documents, to avoid confusion with policies for Doer
Cloud.

:::

Doer lets you configure your server's Terms of Service and
