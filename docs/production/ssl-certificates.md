...
To keep your communications secure, Doer runs over HTTPS only.
...
- `/etc/ssl/private/doer.key` for the private key
...
browsers, but not others and not the Doer mobile and desktop apps.
...
- Alternatively, run a command like `curl -SsI https://doer.example.com`
...
entire Web. Doer offers a simple automation for
...
We recommend most Doer servers use Certbot. You'll want something
else if:
...
- your Doer server is not on the public Internet. (In this case you
  can [still use Certbot][certbot-manual-mode], but it's less
  convenient; and you'll want to ignore Doer's automation.)
...
### At initial Doer install

To enable the Certbot automation when first installing Doer, just
...
Doer server machine to be reachable by that name from the public
Internet.

If you need to configure a multiple domain certificate, you can generate
one as described in the section below after installing Doer.

[doc-install-script]: install.md#step-2-install-doer

### After Doer is already installed

To enable the Certbot automation on an already-installed Doer
...
using the server (e.g., `doer.example.com`), and EMAIL is a contact
address for the server admins. Additional hostnames can also be
specified to issue a certificate for multiple domains.

### How it works

When the Certbot automation in Doer is first enabled, by either
...
Doer configures automatic renewal for you. As a result, a Doer
...
recommended for production, as it is insecure. The Doer desktop and
...
To generate a self-signed certificate when first installing Doer,
just pass the `--self-signed-cert` flag when
[running the install script][doc-install-script].

To generate a self-signed certificate for an already-installed Doer
server, run the following commands:
...
which the Doer app relies on, makes it finicky about the server's TLS
configuration.
...
By default `nginx` (and therefore a Doer server) offers the
