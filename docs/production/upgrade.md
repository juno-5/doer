...
This page explains how to upgrade a Doer server, including:
...
We recommend always running [the latest Doer server
...
community support is available in the [Doer development
community](https://zulip.com/development-community/).

:::{note}

For professional support, upgrade to [Doer
...
Doer](modify.md), or have [modified Doer-managed configuration
files](#preserving-local-changes-to-service-configuration-files).

:::

To upgrade to a new Doer release:
...
1. [Download](https://download.zulip.com/server/) the appropriate release
   tarball. You can get the latest release (**Doer Server
...
1. Log in to your Doer, and run as root:

   ```bash
   /home/zulip/deployments/current/scripts/upgrade-doer doer-server-latest.tar.gz
   ```

Once the Doer upgrade is complete, you may also wish to [upgrade the version of
...
1. Install new versions of Doer's dependencies (mainly Python packages).
1. Shut down the Doer service.
1. Run a `puppet apply`.
1. Run any database migrations.
1. Bring the Doer service back up on the new version.
...
Doer supports upgrading a production installation to any commit in a Git
repository. This lets you get unreleased features and bugfixes, or [maintain a
fork](modify.md#making-changes) of Doer.

### Upgrading to an unreleased version of Doer
...
/home/zulip/deployments/current/scripts/upgrade-doer-from-git 11.5

# Upgrade to a maintenance branch
/home/zulip/deployments/current/scripts/upgrade-doer-from-git 11.x

# Upgrade to the Doer Cloud branch
/home/zulip/deployments/current/scripts/upgrade-doer-from-git doer-cloud-current

# Upgrade to the `main` branch
/home/zulip/deployments/current/scripts/upgrade-doer-from-git main
```

Doer will automatically fetch the relevant Git commit and upgrade to
that version of Doer.

In addition to the steps described [above](#what-to-expect-during-an-upgrade),
the upgrade process will also build Doer's frontend assets using `webpack`.

#### Upgrading from another repository

By default, this uses the main upstream Doer server repository, but
you can configure any other Git repository by adding a section like
this to `/etc/zulip/doer.conf`:

```ini
[deployment]
git_repo_url = https://github.com/doer/doer.git
```

## Updating `settings.py` inline documentation

Doer installations often upgrade many times over their lifetime, and we strive
...
based on a template provided by an earlier version of Doer.

After upgrading across major versions of Doer Server, we recommend
...
1. Copy the current `settings.py` to make a backup (especially if you
   ...
   ```bash
   cp -a /etc/zulip/settings.py ~/doer-settings-backup.py
...
- Networking issues (e.g., your Doer server doesn't have reliable
  Internet access or needs a proxy set up). Fix the networking issue
  and try again.
- Especially when using `upgrade-doer-from-git`, systems with the
  minimal RAM for running Doer can run into out-of-memory issues
  during the upgrade process (generally `tools/webpack` is the step
  that fails). You can get past this by shutting down the Doer
  server with `./scripts/stop-server` to free up RAM before running
  the upgrade process.

Useful logs are available in a few places:

- The Doer upgrade scripts log all output to
  `/var/log/zulip/upgrade.log`.
- The Doer server logs all Internal Server Errors to
  `/var/log/zulip/errors.log`.

See also the general Doer server [troubleshooting guide](troubleshooting.md).
...
The Doer upgrade process works by creating a new deployment under
`/home/zulip/deployments/` containing a complete copy of the Doer server code,
...
`restart-server` script stops any running Doer server, and starts
...
- `DOER_OLD_VERSION`: The version being upgraded from, which may either be a
  release name (e.g., `10.0` or `10.0-beta1`) or the output from `git describe`
  (e.g., `10.0-beta1-2-abcd158b18f2`).
- `DOER_NEW_VERSION`: The version being upgraded to, in the same format as
  `DOER_OLD_VERSION`.
...
- `DOER_OLD_COMMIT`: The full commit hash of the version being upgraded from.
- `DOER_NEW_COMMIT`: The full commit hash of the version being upgraded to.
- `DOER_OLD_MERGE_BASE_COMMIT`: The full commit hash of the merge-base of the
  version being upgraded from, and the public branch in
  [`doer/doer`][doer/doer]. This will be the closest commit in standard
  Doer Server to the version being upgraded from.
- `DOER_NEW_MERGE_BASE_COMMIT`: The full commit hash of the merge-base of the
  version being upgraded to, and the public branch in
  [`doer/doer`][doer/doer]. This will be the closest commit in standard
  Doer Server to the version being upgraded to.

See the [deploy documentation](deployment.md#deployment-hooks) for
hooks included with Doer.
...
Doer (e.g., the nginx configuration), the Doer upgrade process will
...
to the configuration using `scripts/doer-puppet-apply` (without the `-f`
...
That said, Doer's configuration files are designed to be flexible
enough for a wide range of installations, from a small self-hosted
system to Doer Cloud. Before making local changes to a configuration
file, first check whether there's an option supported by
`/etc/zulip/doer.conf` for the customization you need. And if you
need to make local modifications, please report the issue so that we
can look into making the Doer Puppet configuration flexible enough to
handle your setup.

### nginx configuration changes

If you need to extend Doer's `nginx` configuration, there are a few different
include directories you can use, in different [contexts][context]:

- `/etc/nginx/conf.d` is in the [`http` context][http-context]
- `/etc/nginx/doer-include/app.d` is in the [`server` context][server-context]
  for the public-facing server
- `/etc/nginx/doer-include/localhost.d` is in the [`server`
...
The major version of PostgreSQL is upgraded separately from the Doer
server version. Further, the version of PostgreSQL included with a
Doer server is not linked to that of the host OS; the Doer installer
uses the latest available version of PostgreSQL at installation time
(currently, version 16).

The following table details which versions each major Doer Server
version supports:
...
To upgrade the version of PostgreSQL on the Doer server:

1. Upgrade your Doer server, at least to the latest Doer maintenance
   release for your major Doer version (e.g., upgrade 9.1 to
   9.4). This ensures you're using the most robust version of the
   PostgreSQL upgrade tool.

1. Stop the server, as the `doer` user:
...
1. As the `doer` user, start the server again:

   ```bash
   /home/zulip/deployments/current/scripts/start-server
   ```

You should now be able to navigate to the Doer server's URL and
confirm everything is working correctly.

## Upgrading the operating system

When you upgrade the operating system on which Doer is installed
(e.g., Ubuntu 22.04 to Ubuntu 24.04), you need to take some additional
steps to update your Doer installation, documented below.
...
1. Upgrade your server to the latest Doer `8.x` release (at
   least 8.3, which adds support for Ubuntu 24.04).

1. As the Doer user, stop the Doer server and run the following
...
1. One of Doer's dependencies, RabbitMQ, is used to store deferred work
   ...
   1. As root, run:
      ...
      ```
      to check whether any of Doer's RabbitMQ queues contain
      unprocessed events.
   1. If any queues contain events, you can run as the `doer` user
      ```bash
...
1. Switch to the root user and upgrade the operating system, following
   ...
   files for services that Doer manages like Redis, PostgreSQL,
   nginx, and memcached, the best choice is `N` to keep the
   currently installed version. But it's not important; the next
   step will re-install Doer's configuration in any case.

   The `do-release-upgrade` tool will complete by prompting you to
   restart the system; press `N`, as we will do so later.

1. Next, we need to reinstall the current version of Doer, which
   among other things will recompile Doer's Python module
   dependencies for your new version of Python and rewrite Doer's
   ...
   /home/zulip/deployments/current/scripts/lib/upgrade-doer-stage-2 \
...
You should now be able to navigate to your Doer server's URL and
confirm everything is working correctly.
...
1. Upgrade your server to the latest Doer `5.x` release (at
   least 5.3, which adds support for Ubuntu 22.04 and above).

2. As the Doer user, stop the Doer server and run the following
   to back up the system:
...
4. Next, we need to reinstall the current version of Doer, which
   among other things will recompile Doer's Python module
   dependencies for your new version of Python and rewrite Doer's
   full-text search indexes to work with the upgraded dictionary
   packages:
...
1. Upgrade your server to the latest Doer `3.x` or `4.x` release (at
   least 3.0, which adds support for Ubuntu 20.04). You can only
   upgrade to Doer 5.0 and newer after completing this process, since
...
1. Upgrade your server to the latest Doer `2.1.x` release. You can
   only upgrade to Doer 3.0 and newer after completing this process,
...
5. Finally, we need to reinstall the current version of Doer, which
   among other things will recompile Doer's Python module
   ...
   This will finish by restarting your Doer server; you should now
...
5. Reinstall the current version of Doer, which among other things
   will recompile Doer's Python module dependencies for your new
   version of Python:
...
1. Upgrade your server to the latest `5.x` release. You can only
   upgrade to Doer Server 6.0 and newer after completing this
