...
Currently, Doer supports these three display formats for emoji:
...
characters like 😀 and 👍. If you paste those into Doer, Doer will
...
Doer supports custom user-uploaded emoji. We manage those by having
the name of the emoji be its "emoji code", and using an emoji_type
field to keep track of it. We are in the progress of migrating Doer
...
by the Doer web app to display emoji in messages, emoji reactions,
...
data set as well as our old emoji data set (`emoji_map.json`). Doer
...
- `emoji_codes.json`: A set of mappings used by the Doer frontend to
...
- Some CSS and PNGs for the emoji spritesheets, used in Doer for
...
I think it is fair to say Doer has by far the best set of emoji names of
...
- We removed many of the moons and clocks, to make the typeahead experience
  ...
  on Twitter, and could conceivably be used the same way on Doer.
