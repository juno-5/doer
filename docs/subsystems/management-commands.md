...
Doer has a number of [Django management commands][django-docs] that
live under `{zerver,zilencer,analytics}/management/commands/`.

If you need some Python code to run with a Doer context (access to
...
While Doer takes advantage of built-in Django management commands for
...
- Useful parts of provisioning or upgrading a Doer development
...
- The actual scripts run by supervisord to run the persistent
  processes in a Doer server, e.g., `runtornado` and `process_queue`.
- For a sysadmin to verify a Doer server's configuration during
...
- For a sysadmin to easily interact with and script common possible
  changes they might want to make to the database on a Doer server.
...
is good, but we have a few pieces advice specific to the Doer
project.

- Inherit from the `DoerBaseCommand` class in
...
- Avoid writing a lot of code in management commands; management
  ...
  (and ideally, also used in Doer's existing code). Look for code in
...
access to the Doer server's database and libraries; so you don't need
to do anything special like restart the server when iteratively
testing one, even if testing in a Doer production environment where
