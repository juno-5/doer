...
When you're using Doer and you reload, or narrow to a channel, how
does Doer decide where to place you?

Conceptually, Doer takes you to the place where you left off
...
counting unread messages. Here's how Doer currently decides which
...
- The blue cursor box (the "pointer") is around is called the
  "selected" message. Doer ensures that the currently selected
...
of messages), Doer will select the message you clicked on. This
...
other method that doesn't encode a specific message to visit, Doer
...
want. (When finding the "first unread message", Doer ignores unread
...
browser), Doer will preserve the state -- what (if any) narrow the
...
How does Doer decide whether a message has been read by the user?
...
In a Doer development environment, you can use
