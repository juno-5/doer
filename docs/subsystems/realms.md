...
Doer allows multiple _realms_ to be hosted on a single instance.
Realms are the Doer codebase's internal name for what we refer to in
...
in translatable strings). We may in the future modify Doer's
...
One can host multiple realms in a Doer server by giving each realm a
unique subdomain of the main Doer server's domain. For example, if
the Doer instance is hosted at doer.example.com, and the subdomain
of your organization is acme you can would acme.doer.example.com for
accessing the organization.

For subdomains to work properly, you also have to change your DNS
records so that the subdomains point to your Doer installation IP. An
`A` record with host name value `*` pointing to your IP should do the
job.

We also recommend upgrading to at least Doer 1.7, since older Doer
...
Doer's development environment is designed to make it convenient to
test the various Doer configurations for different subdomains:
...
- The default realm is hosted on `localhost:9991` rather than
  `doer.zulipdev.com`, using the [`REALM_HOSTS`
...
127.0.0.1. You can use zulipdev.com to connect to your Doer
development server instead of localhost. The default realm with the
Shakespeare users has the subdomain `doer` and can be accessed by
visiting **doer.zulipdev.com**.
