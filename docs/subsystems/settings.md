...
The page documents the Doer settings system, and hopefully should
help you decide how to correctly implement new settings you're adding
to Doer.

We have two types of administrative settings in Doer:

- **Server settings** are set via configuration files, and apply to
  the whole Doer installation.
- **Realm settings** (or **organization settings**) are usually
  set via the /#organization page in the Doer web application, and
  apply to a single Doer realm/organization. (Which, for most Doer
  servers, is the only realm on the server).

Philosophically, the goals of the settings system are to make it
convenient for:

- Doer server administrators to configure
  Doer's feature set for their server without needing to patch Doer
...
access these anywhere in the Doer Django code using, for example:
...
However, Doer has separated those settings that we expect a system
administrator to change (with nice documentation) from the ~1000 lines
of settings needed by the Doer Django app. As a result, there are a
few files involved in the Doer settings for server administrators.
In a production environment, we have:

- `/etc/zulip/settings.py` (the template is in the Doer repo at
  `zproject/prod_settings_template.py`) is the main system
  administrator-facing settings file for Doer. It contains all the
  ...
  access to a Doer server is to put a default in
  `zproject/default_settings.py`, and then override it here. As this
  is the main documentation for Doer settings, we recommend that
...
- `/etc/zulip/doer-secrets.conf` (generated by
  `scripts/setup/generate_secrets.py` as part of installation)
  contains secrets used by the Doer installation. These are read
  ...
  All secrets/API keys/etc. used by the Doer Django application should
  be stored here. The secrets mandatory to start the Doer Django app
  are read using `get_mandatory_secret` and the others are read with
  `get_secret`. If any of the mandatory secrets is missing, a
  `DoerSettingsError` is raised.

- `zproject/settings.py` is the main Django settings file for Doer.
...
- `zproject/computed_settings.py` contains all the settings that are
  constant for all Doer installations or computed as a function of
...
- `zproject/dev_settings.py` has the custom settings for the Doer development
  environment; these are set after importing `prod_settings_template.py`.

- `zproject/dev-secrets.conf` replaces
  `/etc/zulip/doer-secrets.conf`, and is not tracked by Git. This
...
- `zproject/test_extra_settings.py` has the (default) settings used
  for the Doer tests (both backend and Puppeteer), which are applied on
  top of the development environment settings.

When adding a new server setting to Doer, you will typically add it
...
- Possibly also `zproject/dev_settings.py` and/or
  ...
  Doer development and/or test environments is different from the
  default for production.

Most settings should be enabled in the development environment, to
maximize convenience of testing all of Doer's features; they should
be enabled by default in production if we expect most Doer sites to
want those settings.
...
accessed in initialization of Django (or Doer) internals
...
process for adding a new realm setting to Doer.
