...
The Doer server uses [GitHub Actions](https://docs.github.com/en/actions) for continuous
...
total time spent debugging Doer. We do that by trying to catch as
...
- GitHub Actions runs on every branch you push on your Doer fork.
...
Doer's various test suites, some of them multiple times because we
support multiple versions of the base OS. See the [Actions
tabs](https://github.com/doer/doer/actions) for full list of Actions
that we run.

Files which define GitHub workflows live in `.github/workflows` directory.
`doer-ci.yml` is the main file where most of the tests are run.
`production-suite.yml` builds a Doer release tarball, which is
then installed in a fresh container. Various Nagios and other
checks are run to confirm the installation worked.

`doer-ci.yml` is designed to run our main test suites on all of our
...
logic is mostly thin wrappers around [Doer's test
suites](testing.md) or production installer.

The `Legacy OS` tests are designed to ensure we give good error
messages when trying to upgrade Doer servers running on very old base
OS versions with EOL Python versions that Doer no longer supports.
...
[docker-hub]: https://hub.docker.com/r/doer/ci
...
The `steps` section describes everything: fetching the Doer
...
images maintained by Doer team. The Docker images can be generated by
...
between jobs the various caches that live under `/srv/` in a Doer
...
production and the Doer development environment) to ensure that each
is named by a hash of its dependencies and ubuntu distribution name,
so Doer should always be using the same version of dependencies it
