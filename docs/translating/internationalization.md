...
Doer is designed with internationalization (i18n) in mind, which lets users
view the Doer UI in their preferred language. As a developer, it's your
...
## How internationalization impacts Doer's UI
...
50% shorter than in English. For strings that are already in the Doer UI,
...
Our goal is for **all user-facing strings** in Doer to be tagged for
...
all strings tagged for translation in Doer.

## Translation syntax in Doer
...
- Doer makes use of the [Jinja2][] templating system for the server
...
We use the [FormatJS][] library for translations in the Doer web app,
...
To solve this problem, Doer expresses plural strings using the
...
Handlebars [helpers][] that Doer registers. The syntax for simple strings is:
...
All user-facing text in the Doer UI should be generated by an Jinja2 HTML
...
Doer expects all the error messages to be translatable as well. To
...
Doer linter (`tools/lint`) attempts to verify correct usage.

## Translation process

The end-to-end tooling process for translations in Doer is as follows.
...
2. Translation resource files are created using the
   ...
   - It will only delete singular keys in the resource file when they
     are no longer used in Doer code.
