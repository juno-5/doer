# Minimal configuration to run a Doer application server.
# Default nginx configuration is included in extension app_frontend.pp.
class doer::app_frontend_base {
  include doer::nginx
  include doer::tusd
  include doer::sasl_modules
  include doer::supervisor
  include doer::tornado_sharding
  ...
  doer::safepackage {
    [
      ...
      # Doer servers by default; adding it here explicitly ensures it
  ...
  file { '/etc/nginx/doer-include/app':
    require => Package[$doer::common::nginx],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/doer/nginx/doer-include-frontend/app',
    notify  => Service['nginx'],
  }
  file { '/etc/nginx/doer-include/uploads.types':
    require => Package[$doer::common::nginx],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/doer/nginx/doer-include-frontend/uploads.types',
    notify  => Service['nginx'],
  }
  file { '/etc/nginx/doer-include/app.d/':
    ensure => directory,
    owner  => 'root',
    group  => 'root',
    mode   => '0755',
  }
  file { '/etc/nginx/doer-include/localhost.d/':
    ensure => directory,
    owner  => 'root',
    group  => 'root',
    mode   => '0755',
  ...
  if $loadbalancers != [] {
    file { '/etc/nginx/doer-include/app.d/keepalive-loadbalancer.conf':
      require => File['/etc/nginx/doer-include/app.d'],
      owner   => 'root',
      group   => 'root',
      mode    => '0644',
      source  => 'puppet:///modules/doer/nginx/doer-include-app.d/keepalive-loadbalancer.conf',
      notify  => Service['nginx'],
    }
  } else {
    file { '/etc/nginx/doer-include/app.d/keepalive-loadbalancer.conf':
  ...
  file { '/etc/nginx/doer-include/app.d/accept-loadbalancer.conf':
    # This moved to /etc/nginx/doer-include/trusted-ip, via
    # nginx.pp. This block can be removed once direct Doer upgrades
    # from Doer 11 are no longer supported.
    ensure => absent,
    notify => Service['nginx'],
  }
  file { '/etc/nginx/doer-include/app.d/healthcheck.conf':
    require => File['/etc/nginx/doer-include/app.d'],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/nginx/healthcheck.conf.template.erb'),
    notify  => Service['nginx'],
  }

  file { '/etc/nginx/doer-include/upstreams':
    require => Package[$doer::common::nginx],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/doer/nginx/doer-include-frontend/upstreams',
    notify  => Service['nginx'],
  }
  ...
  if $configured_nginx_resolver == '' {
    ...
    # resolver in /etc/resolv.conf, so only call it is unset in doer.conf
  ...
  file { '/etc/nginx/doer-include/s3-cache':
    require => [
      Package[$doer::common::nginx],
      File['/srv/zulip-uploaded-files-cache'],
    ],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/nginx/s3-cache.template.erb'),
    notify  => Service['nginx'],
  }

  file { '/etc/nginx/doer-include/app.d/uploads-internal.conf':
    ensure  => file,
    require => Package[$doer::common::nginx],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    notify  => Service['nginx'],
    source  => 'puppet:///modules/doer/nginx/doer-include-frontend/uploads-internal.conf',
  }
  ...
  #
  # Because Doer can run in the multiprocess mode with 4GB of memory,
  ...
  # of memory.
  $queues_multiprocess_default = $doer::common::total_memory_mb > 3800
  ...
  if $doer::common::total_memory_mb > 24000 {
    $uwsgi_default_processes = 16
  } elsif $doer::common::total_memory_mb > 12000 {
    $uwsgi_default_processes = 8
  } elsif $doer::common::total_memory_mb > 6000 {
    $uwsgi_default_processes = 6
  } elsif $doer::common::total_memory_mb > 3000 {
  ...
  $tornado_ports = $doer::tornado_sharding::tornado_ports
  ...
  if $custom_ca_path != '' {
    file { '/usr/local/share/ca-certificates/custom-doer-ca.crt':
      ensure => file,
    ...
    exec { 'update-ca-certificates':
      ...
      before      => File["${doer::common::supervisor_conf_dir}/doer.conf"],
      refreshonly => true,
    }
  ...
  file { "${doer::common::supervisor_conf_dir}/doer.conf":
    ensure  => file,
    require => [Package[supervisor], Exec['stage_updated_sharding']],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/supervisor/doer.conf.template.erb'),
  ...
  file { '/etc/zulip/uwsgi.ini':
    ...
    group   => 'root',
    mode    => '0644',
    content => template('doer/uwsgi.ini.template.erb'),
    notify  => Service[$doer::common::supervisor_service],
  }
  doer::sysctl { 'uwsgi':
  ...
  file { '/etc/logrotate.d/doer':
    ensure  => file,
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/logrotate/doer.template.erb'),
  }

  doer::nagios_plugins {'doer_app_frontend': }

  # This cron job does nothing unless RATE_LIMIT_TOR_TOGETHER is enabled.
  doer::cron { 'fetch-tor-exit-nodes':
    minute => '17',
  }
