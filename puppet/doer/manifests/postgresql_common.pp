class doer::postgresql_common {
  ...
  doer::safepackage { $postgresql_packages:
    ensure  => installed,
  ...
  # Use arcane puppet virtual resources to add postgres user to doer group
  @user { 'postgres':
    groups     => ['ssl-cert'],
    membership => minimum,
    require    => $postgresql_user_reqs,
  }
  User <| title == postgres |> { groups +> 'doer' }
}
