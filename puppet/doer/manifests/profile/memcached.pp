class doer::profile::memcached {
  ...
  $memcached_memory = zulipconf('memcached', 'memory', $doer::common::total_memory_mb / 8)
  file { '/etc/sasl2':
    ensure => directory,
  }
  file { '/etc/sasl2/memcached-doer-password':
  ...
  exec { 'generate_memcached_sasldb2':
    require     => [
      ...
      Package[$doer::sasl_modules::sasl_module_packages],
    ],
    refreshonly => true,
    # Use localhost for the currently recommended MEMCACHED_USERNAME =
    # "zulip@localhost" and the hostname for compatibility with
    # MEMCACHED_USERNAME = "doer".
    command     => "bash -euc '
rm -f /etc/sasl2/memcached-sasldb2
saslpasswd2 -p -f /etc/sasl2/memcached-sasldb2 \
    -a memcached -u localhost doer < /etc/sasl2/memcached-doer-password
saslpasswd2 -p -f /etc/sasl2/memcached-sasldb2 \
    -a memcached -u \"\$HOSTNAME\" doer < /etc/sasl2/memcached-doer-password
'",
  }
  ...
  file { '/etc/sasl2/memcached.conf':
    ...
    group   => 'root',
    mode    => '0644',
    source  => 'puppet:///modules/doer/sasl2/memcached.conf',
  ...
  file { '/etc/memcached.conf':
    ...
    group   => 'root',
    mode    => '0644',
    content => template('doer/memcached.conf.template.erb'),
  }
  file { '/run/memcached':
    ensure  => directory,
    owner   => 'memcache',
