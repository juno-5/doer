class doer::profile::rabbitmq {
  ...
  file { '/etc/rabbitmq/rabbitmq.config':
    ...
    group  => 'rabbitmq',
    mode   => '0644',
    source => 'puppet:///modules/doer/rabbitmq/rabbitmq.config',
  ...
  exec { 'warn-rabbitmq-nodename-change':
    command   => "${facts['doer_scripts_path']}/lib/warn-rabbitmq-nodename-change",
  ...
  file { '/etc/rabbitmq/rabbitmq-env.conf':
    ...
    group  => 'rabbitmq',
    mode   => '0644',
    source => 'puppet:///modules/doer/rabbitmq/rabbitmq-env.conf',
    before => Package['rabbitmq-server'],
    notify => [
      Service['rabbitmq-server'],
  ...
  exec { 'configure-rabbitmq':
    command     => "${facts['doer_scripts_path']}/setup/configure-rabbitmq",
    refreshonly => true,
