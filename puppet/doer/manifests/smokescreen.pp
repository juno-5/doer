class doer::smokescreen {
  include doer::supervisor
  include doer::golang

  $version = $doer::common::versions['smokescreen-src']['version']
  $dir = "/srv/zulip-smokescreen-src-${version}"
  $bin = "/usr/local/bin/smokescreen-${version}-go-${doer::golang::version}"

  doer::external_dep { 'smokescreen-src':
    version        => $version,
  ...
  exec { 'compile smokescreen':
    command     => "${doer::golang::bin} build -o ${bin}",
    cwd         => $dir,
  ...
  file { "${doer::common::supervisor_conf_dir}/smokescreen.conf":
    ensure  => file,
    require => [
      Package[supervisor],
      Exec['compile smokescreen'],
    ...
    group   => 'root',
    mode    => '0644',
    content => template('doer/supervisor/smokescreen.conf.erb'),
  ...
  file { '/etc/logrotate.d/smokescreen':
    ...
    group  => 'root',
    mode   => '0644',
    source => 'puppet:///modules/doer/logrotate/smokescreen',
  }
}
