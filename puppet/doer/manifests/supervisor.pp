class doer::supervisor {
  $supervisor_service = $doer::common::supervisor_service

  package { 'supervisor': ensure => installed }

  $system_conf_dir = $doer::common::supervisor_system_conf_dir
  ...
  $conf_dir = $doer::common::supervisor_conf_dir
  file { $conf_dir:
    ensure  => directory,
    require => Package['supervisor'],
    owner   => 'root',
  ...
  if 'docker' in $puppet_classes {
    ...
    service { $supervisor_service:
      ...
      hasstatus  => true,
      status     => $doer::common::supervisor_status,
      ...
      restart    => "bash -c 'if pgrep -x supervisord >/dev/null; then supervisorctl reread && supervisorctl update; else ${doer::common::supervisor_start}; fi'",
      # lint:endignore
    }
    exec { 'supervisor-restart':
      refreshonly => true,
      provider    => shell,
      command     => $doer::common::supervisor_reload,
      require     => Service[$supervisor_service],
    }
  }

  $file_descriptor_limit = zulipconf('application_server', 'service_file_descriptor_limit', 40000)
  file { $doer::common::supervisor_conf_file:
    ensure  => file,
    require => Package[supervisor],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/supervisor/supervisord.conf.erb'),
  ...
  file { '/usr/local/bin/secret-env-wrapper':
    ...
    group  => 'root',
    mode   => '0755',
    source => 'puppet:///modules/doer/secret-env-wrapper',
  }
}
