class doer::tornado_sharding {
  ...
  exec { 'stage_updated_sharding':
    command   => "${facts['doer_scripts_path']}/lib/sharding.py",
    onlyif    => "${facts['doer_scripts_path']}/lib/sharding.py --errors-ok",
  ...
  # the doer.conf configuration. Default is just port 9800.
  ...
  file { '/etc/nginx/doer-include/tornado-upstreams':
    require => [Package[$doer::common::nginx], Exec['stage_updated_sharding']],
    owner   => 'root',
    group   => 'root',
    mode    => '0644',
    content => template('doer/nginx/tornado-upstreams.conf.template.erb'),
    notify  => Service['nginx'],
  }
}
