...
#
class doer::wal_g {
  $wal_g_version = $doer::common::versions['wal-g']['version']
  $bin = "/srv/zulip-wal-g-${wal_g_version}"

  if ($doer::common::goarch == 'aarch64') {
    $package = "wal-g-pg-ubuntu-20.04-${doer::common::goarch}"
  } else {
    $package = "wal-g-pg-ubuntu-22.04-${doer::common::goarch}"
  }
  # This tarball contains only a single file, which is extracted as $bin
  doer::external_dep { 'wal-g':
  ...
  # until we moved to using Doer::External_Dep which places them in
  ...
  file { '/usr/local/bin/env-wal-g':
    ...
    group   => 'root',
    mode    => '0755',
    source  => 'puppet:///modules/doer/postgresql/env-wal-g',
    require => [
      File['/usr/local/bin/wal-g'],
    ],
  }
