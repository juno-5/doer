...
def parse_version_from(deploy_path: str, merge_base: bool = False) -> str:
    if not os.path.exists(os.path.join(deploy_path, "doer-git-version")):
        try:
            ...
            subprocess.check_call(
                [os.path.join(deploy_path, "tools", "cache-doer-git-version")],
                cwd=deploy_path,
                preexec_fn=su_to_doer,
            )
            ...
            pass
    try:
        varname = "DOER_MERGE_BASE" if merge_base else "DOER_VERSION"
...
def get_deployment_version(extract_path: str) -> str:
    ...
    for item in os.listdir(extract_path):
        ...
        if item.startswith("doer-server") and os.path.isdir(item_path):
...
def get_doer_pwent() -> pwd.struct_passwd:
    ...
    # `doer` user as that's the correct value in production.
    return pwd.getpwnam("doer")
...
def get_postgres_pwent() -> pwd.struct_passwd:
    try:
        ...
        return get_doer_pwent()


def su_to_doer(save_suid: bool = False) -> None:
    """Warning: su_to_doer assumes that the doer checkout is owned by
    the doer user (or whatever normal user is running the Doer
...
def get_dev_uuid_var_path(create_if_missing: bool = False) -> str:
    doer_path = get_deploy_root()
    uuid_path = os.path.join(os.path.realpath(os.path.dirname(doer_path)), ".doer-dev-uuid")
    if os.path.exists(uuid_path):
        with open(uuid_path) as f:
            doer_uuid = f.read().strip()
    else:
        if create_if_missing:
            doer_uuid = str(uuid.uuid4())
            # We need root access here, since the path will be under /srv/ in the
            # development environment.
            run_as_root(["sh", "-c", 'echo "$1" > "$2"', "-", doer_uuid, uuid_path])
        else:
            raise AssertionError("Missing UUID file; please run tools/provision!")

    result_path = os.path.join(doer_path, "var", doer_uuid)
...
def log_management_command(cmd: Sequence[str], log_path: str) -> None:
    ...
    logger = logging.getLogger("doer.management")
...
def get_recent_deployments(threshold_days: int | None) -> set[str]:
    ...
    # including `/root/doer` directory if it exists.
    recent = set()
    ...
    if os.path.exists("/root/doer"):
        recent.add("/root/doer")
    return recent
...
def generate_sha1sum_emoji(doer_path: str) -> str:
    sha = hashlib.sha1()

    filenames = [
        "web/images/doer-emoji/doer.png",
    ...
    for filename in filenames:
        file_path = os.path.join(doer_path, filename)
    ...
    with open(os.path.join(doer_path, "node_modules/emoji-datasource-google/package.json")) as fp:
...
def assert_not_running_as_root() -> None:
    ...
    if is_root():
        pwent = get_doer_pwent()
        msg = (
            f"{os.path.basename(script_name)} should not be run as root. Use `su {pwent.pw_name}` to switch to the 'doer'\n"
...
def get_config(
    ...
    if config_file.has_option(section, key):
        ...
        if isinstance(default_value, bool):
            # This list is parallel to puppet/doer/lib/puppet/functions/zulipconf.rb
...
def get_config_file() -> configparser.RawConfigParser:
    config_file = configparser.RawConfigParser()
    config_file.read("/etc/zulip/doer.conf")
    return config_file
...
def has_application_server(once: bool = False) -> bool:
    if once:
        return os.path.exists("/etc/supervisor/conf.d/doer/doer-once.conf")
    return (
        # Current path
        os.path.exists("/etc/supervisor/conf.d/doer/doer.conf")
        # Old path, relevant for upgrades
        or os.path.exists("/etc/supervisor/conf.d/doer.conf")
    )
...
def has_process_fts_updates() -> bool:
    return (
        # Current path
        os.path.exists("/etc/supervisor/conf.d/doer/doer_db.conf")
        # Old path, relevant for upgrades
        or os.path.exists("/etc/supervisor/conf.d/doer_db.conf")
    )
