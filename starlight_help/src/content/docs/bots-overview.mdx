...
* Send content into and out of Doer.
...
Doer natively supports integrations with over one hundred products, and with
...
* The **email** is not used for anything, and will likely be removed in a
  future version of Doer.
* The **bot type** determines what the bot can and can't do (see below).
* The **API key** is how the bot identifies itself to Doer. Anyone with the
...
| Incoming webhook | Limited to only sending messages into Doer                        | Automated notifications into Doer                              |
...
A **generic** bot acts like a normal Doer user that can only access Doer via
...
POST request format can be in a Doer format or a Slack-compatible
format. This is the preferred bot type for interactive bots built on
top of Doer Botserver.
...
Doer organization, but administrators can
...
from your Doer account), use your [personal API key](/api/api-keys), rather
than a bot's API key. You won't need to create a bot.

If you need a bot to send messages on behalf of multiple users, ask [Doer
