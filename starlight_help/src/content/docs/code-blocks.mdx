...
Doer's compose box has a smart **Code** (<CodeIcon />) button, which inserts contextually appropriate code
formatting:
...
(if configured) [code playgrounds](#code-playgrounds). Doer supports syntax
...
Doer code blocks that are tagged with the language will have a button visible
...
<DoerTip>
  ...
  compliant URL templates to describe how links should be generated. Doer's
