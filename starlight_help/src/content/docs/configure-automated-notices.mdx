...
The Doer sends automated notices via Notification Bot to notify users about
...
configure what channel Doer uses for these notices, or disable these notices
...
You can configure whether Doer will send an automated message when a channel's
...
When an administrator modifies your account settings, you will receive a private
message from Notification Bot listing the changes that were made. This includes
updates to your full name, [role](/help/roles-and-permissions), and [custom profiles
fields](/help/custom-profile-fields).

The notification will show both the old and new values for each field that was
changed, making it easy to see exactly what was modified. Only administrators
can change other users' profiles, and these notifications ensure that users are
always informed when such changes occur.

## Doer update announcements

Doer announces new features and other important product changes via automated
messages. This is designed to help users discover new features they may find
useful, including new configuration options.

These announcements are posted to the “Doer updates” topic in the
channel selected by organization administrators (usually 1-2x a month
on Doer Cloud). You can read update messages whenever it's
...
On self-hosted Doer servers, announcement messages are shipped with the Doer
...
Unlike other notices, Doer update announcements are not translated.

### Configure Doer update announcements
...
<FlattenedSteps>
  ...
  1. Under **Automated messages and emails**, configure **Doer update
     announcements**.
...
* [Doer Cloud newsletter](/help/email-notifications#low-traffic-newsletter)
