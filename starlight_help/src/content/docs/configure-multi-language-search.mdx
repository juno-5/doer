...
Doer supports [full-text search](/help/search-for-messages), which can be
combined arbitrarily with Doer's full suite of [search
filters](/help/search-for-messages#search-filters). By default, Doer search
...
Self-hosted Doer organizations can instead set up an experimental
