...
By default, Doer requests a generally-accepted certificate during the
...
which case your Doer server may be using a self-signed
certificate. This is most common for Doer servers not connected to
the public internet.

## Web

Most browsers will show a warning if you try to connect to a Doer server
with a self-signed certificate.

If you are absolutely, 100% sure that the Doer server you are connecting to
...
give a stranger full access to your Doer account, including your
username and password.

## Desktop

Doer Desktop uses the operating system's certificate store, like your web
browser.
...
<Tabs>
  <TabItem label="macOS">
    <Steps>
      ...
      1. Close the window.  You will be prompted for your password to verify
         the change.
      1. Restart the Doer Desktop application.
    </Steps>
  </TabItem>

  <TabItem label="Windows">
    On Windows, Doer Desktop shares the certificate store with
  ...
  <TabItem label="Linux">
    ...
    certutil -d sql:$HOME/.pki/nssdb -A -t "P,," -n doer \
      -i path/to/certificate.pem
    ```

    You will need to restart the Doer Desktop application to pick up the
    new certificate.
  </TabItem>
</Tabs>
