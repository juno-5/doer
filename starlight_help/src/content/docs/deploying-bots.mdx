---
title: Deploying bots in production
---
...
* The simplest is running `doer-run-bot` inside a `screen` session on
...
* Using `supervisord` or a similar tool for managing a production
  process with `doer-run-bot`.  This consumes a bit of resources
  (since you need a persistent process running), but otherwise works
  great.
* Using the Doer Botserver, which is a simple Flask server for
  running a bot in production, and connecting that to Doer's outgoing
  webhooks feature.  This can be deployed in environments like
  Heroku's free tier without running a persistent process.

## Doer Botserver

The Doer Botserver is for people who want to

* run bots in production.
* run multiple bots at once.

The Doer Botserver is a Python (Flask) server that implements Doer's
...
Doer Botserver starts a web server that listens to incoming messages
from your main Doer server. The sequence of events in a successful
...
<Steps>
  ...
  1. The Doer server sends a POST request to your Botserver endpoint URL:
     ...
     This URL is configured in the Doer web-app in your Bot User's settings.

  1. The Botserver searches for a bot to handle the message, and executes your
     bot's `handle_message` code.
</Steps>

Your bot's code should work just like it does with `doer-run-bot`.

### Installing the Doer Botserver

Install the `doer_botserver` package:

```
pip3 install doer_botserver
```

### Create a bot in your Doer organization
...
### Running a bot using the Doer Botserver

<Steps>
  1. [Create your bot](#create-a-bot-in-your-doer-organization) in your Doer
     organization.
  1. Download the `zuliprc` file for the bot created above from the
     **Bots** tab of the **Personal settings** menu, by clicking the download
     (<i class="fa fa-download" />) icon under the bot's name.
  1. Run the Botserver, where `helloworld` is the name of the bot you
     want to run:

     `doer-botserver --config-file <path_to_zuliprc> --bot-name=helloworld`

     You can specify the port number and various other options; run
     `doer-botserver --help` to see how to do this.
</Steps>

Congrats, everything is set up! Test your Botserver like you would
test a normal bot.

### Running multiple bots using the Doer Botserver

The Doer Botserver also supports running multiple bots from a single
Botserver process.

<Steps>
  1. [Create your bots](#create-a-bot-in-your-doer-organization)
     in your Doer organization.

  1. Download the `botserverrc` file from the **Bots** tab of the
     **Personal settings** menu, using the **Download config of all active
     outgoing webhook bots in Doer Botserver format** option.
  ...
  1. Run the Doer Botserver by passing the `botserverrc` to it.

     ```
     doer-botserver --config-file <path-to-botserverrc> --hostname <address> --port <port>
     ```
...
### Running Doer Botserver with supervisord
...
section documents how to run the Doer Botserver using *supervisord*.

Running the Doer Botserver with *supervisord* works almost like
running it manually.
...
<FlattenedSteps>
  ...
  1. Configure *supervisord*.  *supervisord* stores its configuration in
     ...
     * Do **one** of the following:
       * Download the [sample config file][supervisord-config-file]
         and store it in `/etc/supervisor/conf.d/doer-botserver.conf`.
       * Copy the following section into your existing supervisord config file.

         ```
         [program:doer-botserver]
         command=doer-botserver --config-file=<path/to/your/botserverrc>
  ...
  [supervisord-config-file]: https://raw.githubusercontent.com/doer/python-doer-api/main/doer_botserver/doer-botserver-supervisord.conf
  ...
  1. Test if your setup is successful:
     ...
     > doer-botserver                 RUNNING   pid 28154, uptime 0:00:27
...
* Your Botserver needs to be accessible from your Doer server over
  HTTP(S).  Make sure any firewall allows the connection.  We
  recommend using [doer-run-bot](/help/running-bots) instead for
  development/testing on a laptop or other non-server system.
  If your Doer server is self-hosted, you can test by running `curl
  http://zulipbotserver.example.com:5002` from your Doer server;
