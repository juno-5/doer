...
Doer can be configured to send message notification emails for [DMs, mentions,
...
[active](/help/status-and-availability#availability) on Doer. Messages sent to
...
You can reply to Doer messages by replying to message notification emails.
...
You can configure whether the name of your Doer organization is included in the
subject of message notification emails.

Doer offers a convenient **Automatic** configuration option, which includes the
name of the organization in the subject only if you have accounts in multiple
Doer Cloud organizations, or in multiple organizations on the same Doer server.
...
Doer messages from your email. Organization administrators can do this at an
...
By default, Doer sends an email whenever you log in to Doer. These emails
...
In typical usage, these emails are sent infrequently, since all Doer apps
...
Doer sends out a low-traffic newsletter (expect 2-4 emails a year)
to Doer Cloud users announcing major changes in Doer.
...
<Tabs>
  <TabItem label="Doer Cloud">
    <FlattenedSteps>
      ...
      1. Under **Other emails**, toggle
         **Send me Doer's low-traffic newsletter (a few emails a year)**.
    </FlattenedSteps>
  </TabItem>
</Tabs>
...
* [Using Doer via email](/help/using-doer-via-email)
