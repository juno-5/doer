...
import LinkIcon from "~icons/doer-icon/link-2";
import UserCogIcon from "~icons/doer-icon/user-cog";

Many [Doer integrations](/integrations/) are webhooks. An incoming webhook
integration allows a third-party service, such as an [issue
tracker](/integrations/github) or an [alerting
tool](/integrations/pagerduty), to post updates to Doer. To configure
where these updates will be posted, you need to generate a special Doer
integration URL.
