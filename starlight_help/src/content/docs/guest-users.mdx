...
invited to a company's Doer chat.
...
Doer Cloud plans have [special discounted
pricing](/help/doer-cloud-billing#temporary-users-and-guests) for guest users.
...
Doer can display a warning to let users know when recipients for a direct
...
* The user does not appear in typeahead suggestions, e.g., in the compose box
  and search.
* Otherwise, such a user will be displayed as an **Unknown user** in the Doer
