...
You can import your current workspace into a Doer organization. It's a great way
to preserve your workspace history when you migrate to Doer, and to
...
* **Teams** as Doer channels, including all user subscriptions
...
To import your Microsoft Teams data into Doer, you will need to take the following
...
<Steps>
  ...
  1. [Import your Microsoft Teams data into Doer](#import-your-data-into-doer).
...
When planning to migrate from Microsoft Teams to Doer, make sure your Microsoft
tenant is active and still has access to Microsoft services for at least a
couple of months. The process to gain access to the Teams data export tool [can
take around a month](https://blog.zulip.com/2025/09/10/teams-export-vaporware/),
and Doer's import tool for Microsoft Teams requires an active Microsoft Entra
ID application. Additionally, since the imported users' email addresses will be
their Microsoft tenant emails, they will still need access to those accounts to
[update their Doer email addresses](/help/change-your-email-address).

## Import your organization from Microsoft Teams into Doer
...
<Steps>
  1. Make sure that you are an admin of your Microsoft organization. [Request
     access to the data export tool](https://aka.ms/AccessTeamsDataExportTool)
...
Some organization data are not exported by the Teams data export tool. The Doer import
...
* **User roles**. The [user roles](#user-roles) section explains how  Microsoft
  Teams users roles and user types are mapped to Doer user roles.

To access those APIs, create a Microsoft Entra ID application with the required
permissions, and [authorize an access
...
<Steps>
  ...
  1. Navigate to the sidebar → **Entra ID** → **App registrations**, and click **New
     registration**.
  1. In the **Register an application** menu, enter the application's name
     (e.g., "Doer export").
  ...
  1. Click the **Grant admin consent for `your organization`** button in the **API
...
<DoerNote>
  If the token expires and you need a new one, repeat step 14 above.
</DoerNote>

#### Required token permissions

The following Microsoft Graph API endpoints are used by the Doer import
tool:
  ...
  <TabItem label="Self hosting">
    ...
    <FlattenedSteps>
      ...
      1. To import into an organization hosted on the root domain
         (`EXTERNAL_HOST`) of the Doer installation, run the following
...
into Doer. If importing any of this data is important for your organization,
...
Microsoft tenant's user roles are mapped to Doer's [user
roles](/help/user-roles) in the following way:

| Microsoft Teams role | Doer role |
| -------------------- | ---------- |
...
The Teams export tool does not export organization settings, or full user
settings. Configure organization settings as described
[below](#get-your-organization-started-with-doer), and encourage users to
[customize their account settings](/help/getting-started-with-doer).

## Clean up after the Microsoft Teams export

Once your organization has been successfully imported in to Doer, you should
...
<Steps>
  ...
  1. All users from your previous workspace will have accounts in your new Doer
  ...
  1. Share the URL for your new Doer organization, and (recommended) the [Getting
     started with Doer guide](/help/getting-started-with-doer).
  1. **Make sure all users [change their Doer email
     addresses](/help/change-your-email-address) from their Microsoft tenant
     emails** to ones they can continue to access, before your organization's
     Microsoft subscription expires.
