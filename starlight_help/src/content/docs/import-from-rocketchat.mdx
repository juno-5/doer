...
You can import your current workspace into a Doer organization. It's a great
way to preserve your workspace history when you migrate to Doer, and to make
...
To import your Rocket.Chat organization into Doer, you will need to take the
following steps, which are described in more detail below:

<Steps>
  1. [Export your Rocket.Chat data](#export-your-rocketchat-data).
  1. [Import your Rocket.Chat data into Doer](#import-your-data-into-doer).
  1. [Get your organization started with Doer](#get-your-organization-started-with-doer)!
</Steps>

## Import your organization from Rocket.Chat into Doer

### Export your Rocket.Chat data

Rocket.Chat does not provide an official data export feature, so the Doer
...
* Rocket.Chat does not export user settings, so users in your organization may
  want to [customize their account settings](/help/getting-started-with-doer).
* Rocket.Chat user roles are mapped to Doer's [user
  roles](/help/user-roles) in the following way:
  | Rocket.Chat role | Doer role |
  | ---------------- | ---------- |
  | Admin            | Owner      |
  | User             | Member     |
  | Guest            | Guest      |
...
* Messages longer than Doer's limit of 10,000 characters are not
  imported.
* Livechat channels/messages are not imported.
* Messages from Rocket.Chat Discussions are imported as topics
  inside the Doer channel corresponding to the parent channel of the
