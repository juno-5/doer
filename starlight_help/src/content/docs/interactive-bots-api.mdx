...
* intercept, view, and process messages sent by users on Doer.
...
From `doer_bots/bots/followup/followup.py`:
...
`handle_message(self, message, bot_handler)`: Handle a Doer message.

### Arguments

* `self`: The instance the method is called on

* `message`: A dictionary describing a Doer message
...
`bot_handler.send_reply(message, response)`: Reply to a Doer message,
i.e., the response will be sent to the same place that the original
Doer message was sent to.

### Arguments

* `message`: A dictionary containing information on the Doer message
...
From `doer_bots/bots/incrementor/incrementor.py`:
...
The data is stored in the Doer Server's database. Each bot user has
...
* **key**: The API key for the bot (this is how Doer knows
