...
Doer makes it easy to share links to messages, topics, and channels. You can
link from one Doer [conversation](/help/reading-conversations) to another, or
share links to Doer conversations in issue trackers, emails, or other external
tools.

## Link to a channel within Doer

Channel links are automatically formatted as [#channel name](#link-to-a-channel-within-doer).
...
In the web and desktop apps, when you paste a channel link into Doer,
...
## Link to a topic within Doer

Topic links are automatically formatted as [#channel > topic](#link-to-a-topic-within-doer).
...
In the web and desktop apps, when you paste a topic link into Doer, it's
...
## Link to Doer from anywhere

All URLs in Doer are designed to be **shareable**, including:
...
* [Message links](#get-a-link-to-a-specific-message) will still work even when
  ...
  resolved](/help/resolve-a-topic). Doer uses the same permanent link syntax
...
When you copy a Doer link in the web and desktop apps, and paste it anywhere that
accepts HTML formatting (e.g., your email, GitHub, docs, etc.), the link will be
formatted as it would be in Doer (e.g., [#channel > topic](#link-to-a-topic-within-doer)).
To paste the plain URL, you can paste without formatting (likely <kbd>Ctrl</kbd> +
<kbd>Shift</kbd> + <kbd>V</kbd> in your browser).

<DoerNote>
  Due to a Safari quirk, pasting Doer links from Safari into GitHub
...
<Tabs>
  <TabItem label="Desktop/Web">
    ...
    <DoerTip>
      If using Doer in a browser, you can also click on the timestamp
...
In the web and desktop apps, when you paste a message link into Doer,
...
<Tabs>
  <TabItem label="Via message recipient bar">
    <Steps>
      1. Click the **copy link to topic** (<LinkIcon />) icon in the message
         recipient bar.
    </Steps>

    <DoerTip>
      If using Doer in a browser, you can also click on a topic name,
...
* [Linking to your organization](/help/linking-to-doer)
