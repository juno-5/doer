...
<DoerTip>
  This feature is not available on self-hosted Doer servers where
...
You can send emails to Doer channels. This can be useful:

* If you have an email that you want to discuss on Doer
...
<DoerNote>
  You will see the button to generate an email address only if you
  have permission to send messages to this channel.
</DoerNote>
...
<FlattenedSteps>
  ...
  1. Select **Who should be the sender of the Doer messages for this email
     address**.
...
The email subject will become the Doer topic, and the email body will
become the Doer message.
...
You can configure **who should be the sender of the Doer messages** for the
generated email address, with the following options:

* **Email Gateway bot**: This option makes it easy to see that the message
  was sent via email.
* **You**: Messages will look the same as messages you send from the Doer UI.
...
* **The sender's email address**: Adds `From: <Sender email address>` to
  the top of the Doer message.
* **Email footers**: By default, Doer tries to automatically remove some footer
  text (like signatures). With this option enabled, Doer will include all footers.
* **Quoted original email**: In many email clients, when you reply to a message
  ...
  automatically added to the bottom of your reply. By default, Doer tries
  to remove that copied message. With this option enabled, Doer will include it.
* **Use html encoding**: The body of an email is typically encoded using
  ...
  HTML (`text/html`).  Doer supports constructing the Doer message
  content using either (converting HTML to Markdown for the HTML
  format).  By default, Doer will prefer using the plain text version
