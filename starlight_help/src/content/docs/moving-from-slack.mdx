...
import SetUpServerStep from "../include/_SetUpServerStep.mdx";
...
guide on moving to Doer](/help/moving-to-doer).
...
channels, and custom emoji. Data is imported into Doer as a new organization,
so the best time to import is when your team is about to start using Doer for
day-to-day work. This may be part of your evaluation process, or after you've
made the decision to move to Doer.
...
<FlattenedSteps>
  ...
  1. [Create channels](/help/create-channels) if you decided not to import your
     Slack workspace. Doer's [topics](/help/introduction-to-topics) give each
     conversation its own space, so one channel per team should be enough to get
     you started.
  1. Move your integrations using Doer's [Slack-compatible incoming
     webhook](/integrations/slack_incoming). You can transition to
     Doer-native [integrations](/integrations) over time.
</FlattenedSteps>
...
<FlattenedList>
  ...
  * Doer makes it easy to find conversations and follow up. To avoid disrupting
    focus work, @-mentions in Doer should generally be reserved for
...
<FlattenedSteps>
  ...
  1. Pointers to additional resources, such as Doer's [getting started
     guide](/help/getting-started-with-doer), and [the guide on how threading
     works in Doer](/help/introduction-to-topics#what-about-threads) for folks
