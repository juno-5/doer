---
title: Non-webhook integrations
---
...
them. Doer supports several other types of integrations.
...
* **Python script integrations**
  ...
  [Doer Python API distribution](https://github.com/doer/python-doer-api/tree/main/doer),
...
* You should always send messages by POSTing to URLs of the form
  `https://doer.example.com/v1/messages/`.

* We usually build Python script integrations with (at least) 2 files:
  `doer_foo_config.py` containing the configuration for the
...
* You should specify a clear HTTP User-Agent for your integration. The
  ...
  ```
  DoerDesktop/0.7.0 (Ubuntu; 14.04)
  DoerJenkins/0.1.0 (Windows; 7.2)
  DoerMobile/0.5.4 (Android; 4.2; maguro)
  ```
