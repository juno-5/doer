...
Outgoing webhooks allow you to build or set up Doer integrations
which are notified when certain types of messages are sent in Doer.
When one of those events is [triggered](#triggering), the Doer server
will send an HTTP POST [payload](/api/outgoing-webhook-payload) to the
webhook's configured URL.

Outgoing webhooks can be used to power a wide range of Doer
integrations. For example, the [Doer Botserver](/help/deploying-bots#doer-botserver)
  ...
  <TabItem label="Desktop/Web">
    <FlattenedSteps>
      ...
      1. Fill out the fields, with **Endpoint URL** set to the URL you'd like
         Doer to post to, and the outgoing webhook [format](/api/outgoing-webhook-payload)
         you plan on using.
      1. Click **Add**.
    </FlattenedSteps>
  </TabItem>
...
can be configured by the administrator of the Doer server by setting
...
send a reply message into Doer.  Doer's outgoing webhook API
provides a convenient way to do that by simply returning an
appropriate HTTP response to the Doer server.
...
```json
{
    "content": "Hey, we just received **something** from Doer!"
}
```

The `content` field should contain [Doer-flavored Markdown](/help/format-your-message-using-markdown).

Note that an outgoing webhook bot can use the [Doer REST
