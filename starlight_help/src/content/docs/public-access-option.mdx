...
your organization accessible to anyone on the Internet via the Doer
API (details below).
...
The following information about your organization can be accessed via the Doer
...
<DoerNote>
  Self-hosted Doer servers must enable support for web-public channels in their
...
including using Doer's [built-in search](/help/search-for-messages)
...
information available to any new account in the Doer organization,
detailed below.
...
* The **Organization settings** and **Channel settings** menus are not
  ...
  required for Doer to load, and may thus be [accessed via the Doer API][info-via-api].
...
about users who do not participate in web-public channels in the Doer
UI, though they may access it via the Doer API.
...
In order to protect Doer organizations from
bad actors, web-public channels have a few limitations designed to make
Doer an inconvenient target:
...
As a reminder, Doer Cloud organizations are expected to
[moderate content](/help/moderating-open-organizations) to ensure compliance
with [Doer's Rules of Use](https://zulip.com/policies/rules).

## Caveats

* Web-public channels do not yet support search engine indexing. You
  can use [doer-archive](https://github.com/doer/doer-archive) to
  create an archive of a Doer organization that can be indexed by
  search engines.
* The web-public view is not yet integrated with Doer's live-update
