...
Doer supports using SAML authentication for single sign-on, both for Doer
Cloud and self-hosted Doer servers. SAML Single Logout is also supported.
...
If you are [self-hosting](https://zulip.com/self-hosting/) Doer, please follow the
...
<Tabs>
  <TabItem label="Okta">
    <FlattenedSteps>
      ...
      1. Assign the appropriate accounts in the **Assignments** tab. These are the users
         that will be able to log in to your Doer organization.
      1. <SendUsInfo />
  ...
  <TabItem label="OneLogin">
    <FlattenedSteps>
      ...
      1. Set a name and logo and click **Save**. This doesn't affect anything in Doer,
  ...
  <TabItem label="Entra ID (AzureAD)">
    <FlattenedSteps>
      ...
      1. Enter a name (e.g., `Doer Cloud`) for the new Entra ID application,
  ...
  <TabItem label="Keycloak">
    <FlattenedSteps>
      ...
      1. In Keycloak, register a new Client for your Doer organization:
...
You can configure each Doer user's [groups](/help/user-groups) to be updated based
on their groups in your Identity Provider's (IdP's) directory every time they
log in.

Your IdP directory's group names don't have to match the associated Doer group
names (e.g., membership in your IdP's group **finance** can be synced to
membership in the Doer group **finance-department**). See the [technical
documentation][saml-group-sync-readthedocs] on how your IdP's groups are mapped
to Doer groups for details.
  ...
  <TabItem label="Okta">
    <Steps>
      1. Follow the instructions [above](#configure-saml) to configure SAML, and go to
         the application you created for using SAML with Doer in your
         **Applications** menu.
      ...
      1. Add the following attribute:
         ...
         * **Filter**: `Matches regex: .*`
           When a user signs in to Doer via SAML, Okta will now include a list of the
           user's groups in its response to the Doer server.
      1. To enable this feature, please email
         [support@zulip.com](mailto:support@zulip.com) with the following information:
         * Your Doer organization URL.
         * Which groups should be synced from your IdP's directory.
         * Which groups should have a different name in Doer (if any).
    </Steps>
  </TabItem>
</Tabs>
