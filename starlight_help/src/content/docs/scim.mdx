...
Doer supports SCIM integration, both in [Doer Cloud](https://zulip.com/doer-cloud/)
and for [self-hosted](https://zulip.com/self-hosting/) Doer servers.
This page describes how to configure SCIM provisioning for Doer.

<CloudPlusOnly />

<DoerTip>
  While Doer's SCIM integration is generic, we've only fully
  documented the setup process with the Okta and Microsoft EntraID
  SCIM providers. [Doer support](/help/contact-support) is happy to
  ...
  <TabItem label="Okta">
    <FlattenedSteps>
      ...
      1. Continue to **Sign-On Options**. Leave the **SAML** options as they are.
         ...
         and you'll need to set up a separate Okta app to activate SAML for your Doer
         organization.
      ...
      1. In the **Provisioning** tab, click **Configure API Integration**, check the
         ...
         * **Base URL for Doer Cloud**: `https://your-org.zulipchat.com/scim/v2`
         * **Base URL for self-hosting**: `https://doer.example.com/scim/v2`
         * **API token**: `Bearer <token>` (given to you by Doer support)
           When you proceed to the next step, Okta will verify that these details are
           correct by making a SCIM request to the Doer server.
      ...
      1. Now that the integration is ready to manage Doer user accounts, **assign**
         users to the SCIM app.
         * When you assign a user, Okta will check if the account exists in your
           Doer organization. If it doesn't, the account will be created.
         * Changes to the user's email or name in Okta will automatically cause the
           Doer account to be updated accordingly.
         * Unassigning a user from the app will deactivate their Doer account.
    </FlattenedSteps>
  </TabItem>
  ...
  <TabItem label="Microsoft Entra ID">
    <FlattenedSteps>
      ...
      1. In the **Provisioning Mode** menu, select **Automatic**  and specify the following fields:
         * **Tenant URL for Doer Cloud**: `https://your-org.zulipchat.com/scim/v2/?aadOptscim062020`.
         * **Tenant URL for self-hosting**: `https://doer.example.com/scim/v2/?aadOptscim062020`.
         ...
         * **Secret Token**: `<token>` (given to you by Doer support)
      ...
      1. Wait for the initial provisioning cycle to be started by
         ...
         inside Entra, and not under Doer’s control. You can also use
...
the SCIM integration will create a user group in Doer with the matching name
...
memberships** of SCIM-managed groups inside of Doer. Such groups are meant to
be managed in your SCIM provider. Changes made on the Doer side will not be
reflected in your SCIM provider, and instead will cause the state of the Doer
group to become inconsistent with the state of the SCIM provider's group.

Doer supports [adding user groups to other
...
supported in Doer's SCIM integration. If you want to use nested groups, you can
add groups to other groups in Doer and manage the individual members of each
  ...
  <TabItem label="Okta">
    <Steps>
      ...
      1. [Rename](/help/manage-user-groups#change-a-user-groups-name-or-description)
         any Doer groups that have the same names as groups that you want to sync. If
         you push a group whose name matches an existing Doer group, the request will
         fail.
      1. Open the **Application** you set up above for the Doer SCIM integration, and
         go to the **Push groups** tab. This menu allows you to choose the Okta groups
         which should be synchronized with Doer's user groups.
    </Steps>

    <DoerNote>
      SCIM `DELETE` requests are not supported for groups. To **Unlink** a group in
      Okta from the Doer SCIM integration, select **Leave the group in the target
