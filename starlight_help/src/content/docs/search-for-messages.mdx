...
import SearchIcon from "~icons/doer-icon/search";

It's easy to find the right conversation with Doer's powerful search. When you
find the message you were looking for, go directly to its topic for full context.

Whenever you need a reminder of the search filters that Doer offers, check out
the convenient [**search filters**](#search-filters-reference) reference
in the Doer web and desktop apps.
...
Doer lets you search messages and topics by keyword. For example:
...
* Doer will find messages containing similar keywords (keywords with the same
  stem), so, e.g., `wave` will match `waves` and `waving`.
* Doer search ignores very common words like `a`, `the`, and about 100 others.
...
Doer also offers a wide array of search filters, which can be used on their
...
<DoerTip>
  As you start typing into the search box, Doer will suggest search filters
  you can use.
</DoerTip>
...
Doer offers the following filters based on the location of the message.
...
To avoid cluttering your search results, by default, Doer searches just the
...
Each message in Doer has a unique ID, which is used for [linking to a specific
message](/help/link-to-a-message-or-conversation#link-to-doer-from-anywhere).
...
All of Doer's search filters can be negated to **exclude** messages matching
...
When you search Doer, the URL shown in the address bar of the Doer web app is a
...
A summary of the search filters above is available in the Doer app.
