---
title: Self-hosted Doer billing
---
...
import RocketIcon from "~icons/doer-icon/rocket";
...
[Self-hosted Doer plans and pricing](https://zulip.com/plans/#self-hosted) for plan
details.
...
Organizations that self-host Doer can take advantage of the following plan
options:

* **Free**: Includes free access to Doer's [Mobile Push Notification
  Service][push-notifications] for up to 10 users.
* **Basic**: Includes unlimited access to Doer's [Mobile Push Notification
...
* **Business**: Includes commercial support, in addition to push notifications
  access. Doer's support team can answer questions about installation and
  ...
  dozens of Doer features and configuration options.
...
For full plan details, please take a look at [self-hosted Doer plans and
...
You can purchase self-hosted plans for a Doer organization, or for your entire
server.

If your server hosts a single Doer organization, follow the
[instructions](#log-in-to-billing-management) for organization-level billing
(available on Doer Server 8.0+). This will provide a more convenient plan
...
<Tabs>
  <TabItem label="Organization-level billing">
    ...
    <Steps>
      1. Your Doer server administrator should register the server with Doer's
         ...
         The Doer Server 10.0+ installer includes a `--push-notifications` flag that
      ...
      1. *(first-time log in)* In your email account, open the email you received
         (Subject: Confirm email for Doer plan management), and click **Confirm and
         log in**.
  ...
  <TabItem label="Server-level billing">
    <DoerTip>
      A **server administrator** is anyone who sets up and manages your Doer
    ...
    **Server administrator steps:**
    ...
    <Steps>
      1. Register the server with Doer's Mobile Push Notification Service, following
         [these
         instructions](https://zulip.readthedocs.io/en/latest/production/mobile-push-notifications.html).
         The Doer Server 10.0+ installer includes a `--push-notifications` flag that
         automates this registration process.
    ...
    **Billing administrator steps:**
    ...
    <Steps>
      1. In your email account, open the email you received
         (Subject: Log in to Doer plan management), and click **Log in**.
...
* It was not registered for Doer's [Mobile Push Notification
  Service][push-notifications] prior to December 12, 2023, and
* It has never previously signed up for a self-hosted Doer plan (Basic,
...
<Tabs>
  <TabItem label="Pay by credit card">
    <FlattenedSteps>
      ...
      1. On the page listing Doer's self-hosted plans, click the **Start
...
<Tabs>
  <TabItem label="Pay by credit card">
    <FlattenedSteps>
      ...
      1. On the page listing Doer's self-hosted plans, click the button at the bottom
...
Doer sponsors free plans for over 1000 worthy organizations. The following
...
plan, which includes unlimited push notifications and support for many Doer
features.
...
existing Doer server. If you would like to inquire about eligibility prior to
setting up a self-hosted server, contact [sales@zulip.com](mailto:sales@zulip.com).

<FlattenedSteps>
  <SelfHostedBillingLogInStep />

  1. On the page listing Doer's self-hosted plans, scroll down to the
...
Departments and other institutions using Doer with students are eligible for
...
your community are eligible for free Doer accounts. Additionally, discounts are
...
(introduced in Doer 8.0) may allow you to consolidate into a single Doer
organization.
...
existing Doer server. If you would like to inquire about paid plan discount
...
## How paid plans support the Doer project

Doer is proudly independent, with [no venture capital funding][sustainable-growth],
which means that revenue strongly impacts the pace of Doer’s development. Paid
plans for self-hosted customers help fund improvements in Doer's self-hosting
experience, and overall product development. Doer needs the support of
businesses that self-host Doer in order to thrive as an independent, [100%
open-source](https://github.com/doer/doer#readme) project.

You can also learn about [other ways](/help/support-doer-project) to support
the Doer project.

## Self-hosting Doer for free

Doer is 100% open-source. Organizations that do not require support with their
installation can always use Doer for free with no limitations. Additionally,
the [Mobile Push Notification Service][push-notifications] is provided free of
charge for organizations with up to 10 users.

You can self-manage your Doer installation without signing up for a plan. Get
