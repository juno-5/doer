...
import DoerIconCircle from "../../../../static/images/logo/doer-icon-circle.svg";
...
import AttachmentIcon from "~icons/doer-icon/attachment";
import MobileCameraIcon from "~icons/doer-icon/mobile-camera";
import MobileImageIcon from "~icons/doer-icon/mobile-image";

Doer supports attaching multiple files to messages, including images,
...
<Tabs>
  <TabItem label="Via drag-and-drop">
    <FlattenedSteps>
      1. Drag and drop one or more files anywhere in the Doer app,
         whether or not the compose box is open.
         Doer will upload the files, and insert named links using
    ...
    <FlattenedSteps>
      ...
      1. Tap the
         ...
         **image** (<MobileImageIcon />),
         ...
         button at the bottom of the app to select one or more files. Doer will
...
<Tabs>
  <TabItem label="Android">
    <Steps>
      1. In another mobile app, select text or one or more files, and tap the
         **share** button.
      1. Tap the **Doer** (<img src={DoerIconCircle.src} alt="logo" class="help-center-icon" />) logo to share to Doer.
  ...
  <TabItem label="iOS">
    ...
    tracked [on GitHub](https://github.com/doer/doer-flutter/issues/54).
...
The Doer Cloud Standard and Doer Cloud Plus
[plans](https://zulip.com/plans/#cloud) include 5 GB of file storage per user.
Each uploaded file can be up to 1 GB.

The Doer Cloud Free [plan](https://zulip.com/plans/#cloud) includes a total of
