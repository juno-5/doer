...
import UserCircleActiveIcon from "~icons/doer-icon/user-circle-active";
import UserCircleIdleIcon from "~icons/doer-icon/user-circle-idle";
import UserCircleOfflineIcon from "~icons/doer-icon/user-circle-offline";
...
Your **availability** is a colored dot (like <UserCircleActiveIcon />) that indicates if you're currently active on Doer, idle,
...
* **Active** (<UserCircleActiveIcon />): Doer is open and in focus on web,
  desktop or mobile, or was in the last 140 seconds.
* **Idle** (<UserCircleIdleIcon />): Doer is open on your computer (either
  desktop or web), but you are not active.
* **Offline** (<UserCircleOfflineIcon />): Doer is not open on your computer,
...
Doer supports the privacy option of never updating the availability
...
<DoerTip>
  Consider also [not
  allowing](/help/read-receipts#configure-whether-doer-lets-others-see-when-youve-read-messages)
