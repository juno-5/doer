Fetch tweets from Twitter in Doer! This is great for seeing and
...
1.  Create a `~/.doer_twitterrc` with the following contents:
...
1.  When someone tweets a message containing one of your search terms,
    get a Doer on your specified stream, with the search term as
    the topic.
...
`/etc/zulip/settings.py` on the Doer server.
