...
This directory contains scripts for automating the release of Doer one click apps.

## DigitalOcean 1-Click Application

`prepare_digital_ocean_one_click_app_release.py` creates the image of DigitalOcean 1-Click
app from the latest Doer release (fetched from https://download.zulip.com/server/). It will
also create a test droplet from the image and send the image and droplet
details to a pre-configured Doer stream. Anyone, whose key is added to the
Doer DigitalOcean team can SSH into the droplet for testing.

### Running as GitHub action

`.github/workflows/update-oneclick-apps.yml` is configured to invoke
`prepare_digital_ocean_one_click_app_release.py` as a GitHub action during each Doer
server release.
...
- `ONE_CLICK_ACTION_DOER_BOT_API_KEY` - The API key of the Doer bot used for sending messages.
- `ONE_CLICK_ACTION_DOER_BOT_EMAIL` - The email of the Doer bot.
...
- Use the link generated by the installer to create a new Doer organization. Do some basic
...
- Delete the test droplet `oneclickapp-{release_version}-test` after you have completed testing
  by going to the [DigitalOcean Doer team account](https://cloud.digitalocean.com/droplets?i=0242e0).
...
- Include the fix in the Fabric script that setups the installer.
  [01-initial-setup](https://raw.githubusercontent.com/doer/marketplace-partners/master/marketplace_docs/templates/Fabric/scripts/01-initial-setup)
  file should be a good place to include the fix. See
  [doer/marketplace-partners#4](https://github.com/doer/marketplace-partners/pull/4/files) for an
  example fix.
