...
doer_update_announcements: list[DoerUpdateAnnouncement] = [
    DoerUpdateAnnouncement(
        level=1,
        message="""
Doer is introducing **Doer updates**! To help you learn about new features and
configuration options, this topic will receive messages about important changes in Doer.
...
announcements, they can be disabled. [Learn more]({doer_update_announcements_help_url}).
""".format(
            doer_update_announcements_help_url="/help/configure-automated-notices#doer-update-announcements",
...
- When you paste content into the compose box, Doer will now do its best to preserve
...
""".format(
    ...
    DoerUpdateAnnouncement(
        level=4,
        message="""
- To simplify Doer for new users, **Streams** have been renamed to **Channels**.
...
- You can also [configure]({automatically_go_to_conversation_help_url}) whether Doer
...
""".format(
    ...
    DoerUpdateAnnouncement(
        ...
        message=(
            """
- New image uploads now load much faster in all Doer apps.
...
We make many improvements to Doer beyond what we can share here. Learn about
additional feature highlights, and other Doer project updates since December
...
We make many improvements to Doer beyond what we can share here. Check out our
[release announcement blog post]({blog_post_9_0_url}) to learn about additional
feature highlights in Doer Server 9.0, and other Doer project updates.
"""
            )
        ).format(
            image_previews_help_url="/help/allow-image-link-previews",
            view_images_help_url="/help/view-images-and-videos",
            blog_post_9_0_url="https://blog.zulip.com/doer-server-9-0",
        ),
    ),
    DoerUpdateAnnouncement(
        level=9,
...
- You can now [upload large files]({file_upload_limits_help_url}) up to
  1 GB in organizations on Doer Cloud
  Standard or Doer Cloud Plus [plans]({cloud_plans_url}).
"""
                if settings.CORPORATE_ENABLED
                else """
- You can now [upload large files]({file_upload_limits_help_url}), up to
...
"""
    ...
    DoerUpdateAnnouncement(
        ...
        message=(
            """
- Most permissions in Doer can now be granted to any combination of
  ...
  configurable only by user role."""
            + (
                """
- Creating new user groups now requires a Doer Cloud Standard or Doer Cloud
...
"""
    ...
    DoerUpdateAnnouncement(
        level=11,
        message="""
- Doer’s next-gen mobile app is now in public beta. If offers a sleek new
...
""".format(
    ...
    DoerUpdateAnnouncement(
        level=12,
        message="""
- When you [link to a topic]({link_from_anywhere_help_url}) in Doer, that link
...
""".format(
            link_from_anywhere_help_url="/help/link-to-a-message-or-conversation#link-to-doer-from-anywhere",
    ...
    DoerUpdateAnnouncement(
        level=13,
        message="""
- Doer channels have a new space for **chatting without a topic** (e.g., social
...
**Web and desktop updates**
- When you copy a Doer link and paste it anywhere that accepts HTML formatting
  (e.g., your email, GitHub, docs, etc.), the link will be formatted as it would
  be in Doer (e.g., [#channel > topic]({link_to_a_message_or_conversation_url})).
...
and [line spacing]({line_spacing_url}) options, to make the Doer interface
...
""".format(
    ...
    DoerUpdateAnnouncement(
        ...
        message=(
            """
We make many improvements to Doer beyond what we can share here. Learn about
additional feature highlights, and other Doer project updates since July
...
feature highlights in Doer Server 10.0, and other Doer project updates.
"""
        ).format(
            blog_post_10_0_url="https://blog.zulip.com/doer-server-10-0",
        ),
    ),
    DoerUpdateAnnouncement(
        level=16,
...
""".format(
    ...
    DoerUpdateAnnouncement(
        level=17,
        message="""
Starting this week, you’ll get Doer’s next-generation mobile app the next time
your Doer app is upgraded. The new app offers a sleek new design and a faster,
...
""".format(
    ...
    DoerUpdateAnnouncement(
        level=19,
        message="""
- Doer's topics help you keep conversations organized, but you may not need
...
- Your [home view]({home_view_url}) menu now has an option to [mark as
  ...
  you don't follow. Doer is snappier when you don't have thousands of unread
  messages.
""".format(
...
- Administrators can now [configure]({configure_a_custom_welcome_message_url}) a
  ...
  purpose of important channels, and link to your guidelines for using Doer.
...
- Android users can now [share]({share_and_upload_files_url}) files and text
  from other applications into Doer.
...
""".format(
    ...
    DoerUpdateAnnouncement(
        ...
        message=(
            """
We make many improvements to Doer beyond what we can share here. Learn about
additional feature highlights, and other Doer project updates since March,
...
feature highlights in Doer Server 11.0, and other Doer project updates.
"""
        ).format(
            blog_post_11_0_url="https://blog.zulip.com/doer-server-11-0",
        ),
    ),
    DoerUpdateAnnouncement(
        level=22,
...
def get_latest_doer_update_announcements_level() -> int:
    latest_doer_update_announcement = doer_update_announcements[-1]
    return latest_doer_update_announcement.level


def get_doer_update_announcements_message_for_level(level: int) -> str:
    doer_update_announcement = doer_update_announcements[level - 1]
    return remove_single_newlines(doer_update_announcement.message)


def get_realms_behind_doer_update_announcements_level(level: int) -> QuerySet[Realm]:
    ...
    realms = Realm.objects.filter(
        Q(doer_update_announcements_level__isnull=True)
        | Q(doer_update_announcements_level__lt=level),
        deactivated=False,
    ).exclude(string_id=settings.SYSTEM_BOT_REALM)
    return realms


def get_topic_name_for_doer_update_announcements(realm: Realm) -> str:
    assert realm.doer_update_announcements_stream is not None

    with override_language(realm.default_language):
        topic_name = str(realm.DOER_UPDATE_ANNOUNCEMENTS_TOPIC_NAME)

    if (
        realm.doer_update_announcements_stream.topics_policy
...
def internal_prep_group_direct_message_for_old_realm(
    ...
    if realm.doer_update_announcements_stream is None:
        content = """
Doer now supports [configuring]({organization_settings_url}) a channel where Doer will
send [updates]({doer_update_announcements_help_url}) about new Doer features.
...
""".format(
        ...
        topic_name = get_topic_name_for_doer_update_announcements(realm)
        content = """
Starting tomorrow, users in your organization will receive [updates]({doer_update_announcements_help_url})
about new Doer features in #**{doer_update_announcements_stream}>{topic_name}**.
...
""".format(
            doer_update_announcements_help_url="/help/configure-automated-notices#doer-update-announcements",
            doer_update_announcements_stream=realm.doer_update_announcements_stream.name,
...
def get_level_none_to_initial_auditlog(realm: Realm) -> RealmAuditLog | None:
    return RealmAuditLog.objects.filter(
        ...
        extra_data__contains={
            ...
            "property": "doer_update_announcements_level",
        },
    ).first()
...
def is_realm_imported_from_other_product(realm: Realm) -> bool:
    ...
    # like regular doer imports.
    return import_source is not None and import_source != "doer"


def internal_prep_doer_update_announcements_stream_messages(
    current_level: int, latest_level: int, sender: UserProfile, realm: Realm
) -> list[SendMessageRequest | None]:
    message_requests = []
    stream = realm.doer_update_announcements_stream
    assert stream is not None
    topic_name = get_topic_name_for_doer_update_announcements(realm)

    while current_level < latest_level:
        content = get_doer_update_announcements_message_for_level(level=current_level + 1)
...
def send_messages_and_update_level(
    realm: Realm,
    new_doer_update_announcements_level: int,
    ...
    RealmAuditLog.objects.create(
        ...
        extra_data={
            RealmAuditLog.OLD_VALUE: realm.doer_update_announcements_level,
            RealmAuditLog.NEW_VALUE: new_doer_update_announcements_level,
            "property": "doer_update_announcements_level",
            "doer_update_announcements_message_ids": sent_message_ids,
        },
    )

    realm.doer_update_announcements_level = new_doer_update_announcements_level
    realm.save(update_fields=["doer_update_announcements_level"])


def send_doer_update_announcements(skip_delay: bool, progress: bool = False) -> None:
    latest_doer_update_announcements_level = get_latest_doer_update_announcements_level()
    realms = get_realms_behind_doer_update_announcements_level(
        level=latest_doer_update_announcements_level
    )
    if len(realms) == 0:
        return
    ...
    for i, realm in enumerate(realms, start=1):
        try:
            send_doer_update_announcements_to_realm(realm, skip_delay)
...
def send_doer_update_announcements_to_realm(realm: Realm, skip_delay: bool) -> None:
    latest_doer_update_announcements_level = get_latest_doer_update_announcements_level()
    ...
    realm_doer_update_announcements_level = realm.doer_update_announcements_level
    assert (
        realm_doer_update_announcements_level is None
        or realm_doer_update_announcements_level < latest_doer_update_announcements_level
    )

    sender = get_system_bot(settings.NOTIFICATION_BOT, realm.id)

    messages = []
    new_doer_update_announcements_level = None

    if realm_doer_update_announcements_level is None:
        # This realm predates the doer update announcements feature, or
        # was imported from another product (Slack, Mattermost, etc.).
        # Group DM the administrators to set or verify the stream for
        # doer update announcements.
        ...
        if realm_imported_from_other_product:
            new_doer_update_announcements_level = latest_doer_update_announcements_level
        else:
            new_doer_update_announcements_level = 0
    elif realm.doer_update_announcements_stream is None:
        ...
        # after sending group DMs to let admins configure stream for doer update announcements.
        ...
        if level_none_to_initial_auditlog is None or not (
            ...
            new_doer_update_announcements_level = latest_doer_update_announcements_level
    elif realm.doer_update_announcements_stream.deactivated:  # nocoverage
        # `doer_update_announcements_stream` should be set to None when its channel is
        # deactivated. If this condition occurs, there's a bug that needs investigation.
        raise JsonableError(_("`doer_update_announcements_stream` is unexpectedly deactivated."))
    else:
        # Wait for 24 hours after sending group DM to allow admins to change the
        # stream for doer update announcements from it's default value if desired.
        if (
            (realm_doer_update_announcements_level == 0 or realm_imported_from_other_product)
...
this topic will receive messages about important changes in Doer.
...
they can be disabled. [Learn more]({doer_update_announcements_help_url}).
"""
            ).format(
                doer_update_announcements_help_url="/help/configure-automated-notices#doer-update-announcements",
                mute_topic_help_url="/help/mute-a-topic",
        ...
        messages.extend(
            internal_prep_doer_update_announcements_stream_messages(
                current_level=realm_doer_update_announcements_level,
                latest_level=latest_doer_update_announcements_level,
                sender=sender,
                realm=realm,
            )
        )
    ...
    if new_doer_update_announcements_level is not None:
        send_messages_and_update_level(realm, new_doer_update_announcements_level, messages)
