...
from zerver.lib.doer_update_announcements import (
...
class DoerUpdateAnnouncementsTest(DoerTestCase):
    @override
    def setUp(self) -> None:
        super().setUp()

        self.doer_update_announcements = [
            DoerUpdateAnnouncement(
                level=1,
    ...
    def test_send_doer_update_announcements(self) -> None:
        with mock.patch(
            "zerver.lib.doer_update_announcements.doer_update_announcements",
            self.doer_update_announcements,
        ):
            realm = get_realm("doer")

            # realm predates the "doer updates" feature with the
            # doer_update_announcements_stream set to None.
            realm.doer_update_announcements_level = None
            realm.doer_update_announcements_stream = None
            realm.save(
                update_fields=[
            ...
            with time_machine.travel(now, tick=False):
                send_doer_update_announcements(skip_delay=False)
            ...
            self.assertEqual(realm.doer_update_announcements_level, 0)
            self.assertIn(
            ...
            with time_machine.travel(now + timedelta(days=8), tick=False):
                send_doer_update_announcements(skip_delay=False)
            realm.refresh_from_db()
            self.assertEqual(realm.doer_update_announcements_level, 2)
            ...
            self.doer_update_announcements.extend(new_updates)

            # verify doer update announcements sent to configured stream.
            ...
            self.assertEqual(realm.doer_update_announcements_level, 4)
            self.assertEqual(
                stream_messages[0].topic_name(), realm.DOER_UPDATE_ANNOUNCEMENTS_TOPIC_NAME
            )
            self.assertEqual(
                stream_messages[1].topic_name(), realm.DOER_UPDATE_ANNOUNCEMENTS_TOPIC_NAME
            )
            self.assertEqual(
                stream_messages[2].topic_name(), realm.DOER_UPDATE_ANNOUNCEMENTS_TOPIC_NAME
            )
            ...
            # Calling send_doer_update_announcements again does nothing
    ...
    def test_send_doer_update_announcements_with_stream_configured(self) -> None:
        with mock.patch(
            "zerver.lib.doer_update_announcements.doer_update_announcements",
            self.doer_update_announcements,
        ):
            ...
            # realm predates the "doer updates" feature with the
            # doer_update_announcements_stream configured.
            realm.doer_update_announcements_level = None
            realm.doer_update_announcements_stream = get_stream("verona", realm)
            realm.save(
                update_fields=[
                    "doer_update_announcements_level",
                    "doer_update_announcements_stream",
            ...
            stream_messages = Message.objects.filter(
                realm=realm,
                sender=notification_bot,
                recipient__type_id=realm.doer_update_announcements_stream.id,
            ...
            # Note: 'do_deactivate_stream' sets 'doer_update_announcements_stream' to None.
            channel_id = realm.doer_update_announcements_stream.id
            do_deactivate_stream(realm.doer_update_announcements_stream, acting_user=None)
    ...
    def test_send_doer_update_announcements_skip_delay(self) -> None:
        with mock.patch(
            "zerver.lib.doer_update_announcements.doer_update_announcements",
            self.doer_update_announcements,
        ):
            ...
            # 'send_doer_update_announcements' management command with
            # '--skip-delay' argument to immediately send update messages.
            # 'doer_update_announcements_stream' should be configured.
            with time_machine.travel(now, tick=False):
                send_doer_update_announcements(skip_delay=True)

            realm.refresh_from_db()
            self.assertEqual(realm.doer_update_announcements_level, 2)

    def test_group_direct_message_with_doer_updates_stream_set(self) -> None:
        realm = get_realm("doer")

        # realm predates the "doer updates" feature.
        realm.doer_update_announcements_level = None
        realm.save(update_fields=["doer_update_announcements_level"])

        self.assertIsNotNone(realm.doer_update_announcements_stream)
        ...
        self.assertIn(
            ...
            "[updates](/help/configure-automated-notices#doer-update-announcements) about new Doer features in "
            f"#**{realm.doer_update_announcements_stream}>{realm.DOER_UPDATE_ANNOUNCEMENTS_TOPIC_NAME}**",
            group_direct_message.content,
        )
    ...
    def test_doer_updates_for_realm_imported_from_other_product(self) -> None:
        with mock.patch(
            "zerver.lib.doer_update_announcements.doer_update_announcements",
            self.doer_update_announcements,
        ):
            ...
            # * doer_update_announcements_level = latest level
            # * doer_update_announcements_stream = gryffindor_common_room - channel with the most subscribers,
            #   with earliest id as tiebreaker.
            # * announcements sent to the channel.
            self.assertEqual(imported_realm.doer_update_announcements_level, 2)
            self.assertEqual(
                imported_realm.doer_update_announcements_stream, gryffindor_common_room
            )
            ...
            with time_machine.travel(now + timedelta(hours=25), tick=False):
                send_doer_update_announcements(skip_delay=False)
            imported_realm.refresh_from_db()
            self.assertEqual(imported_realm.doer_update_announcements_level, 4)
            doer_updates_message_query = Message.objects.filter(
                realm=imported_realm,
                sender=notification_bot,
                recipient__type=Recipient.STREAM,
            )
            self.assertTrue(doer_updates_message_query.exists())

    def test_send_doer_update_announcements_new_realm(self) -> None:
        with mock.patch(
            "zerver.lib.doer_update_announcements.doer_update_announcements",
            self.doer_update_announcements,
        ):
            ...
            self.assertEqual(new_realm.doer_update_announcements_level, 2)
            assert new_realm.doer_update_announcements_stream is not None
            ...
            stream_messages = Message.objects.filter(
                realm=new_realm,
                sender=notification_bot,
                recipient__type_id=new_realm.doer_update_announcements_stream.id,
                date_sent__gte=now,
            ).order_by("id")
            ...
            self.assertIn("To help you learn about new features", stream_messages[0].content)
            self.assertEqual(stream_messages[1].content, "Announcement message 3.")
            self.assertEqual(new_realm.doer_update_announcements_level, 3)

            new_realm.doer_update_announcements_stream = None
            new_realm.save(update_fields=["doer_update_announcements_stream"])
            ...
            self.assert_length(stream_messages, 0)
            self.assertEqual(new_realm.doer_update_announcements_level, 4)
