...
class ChangeSettingsTest(DoerTestCase):
    ...
    def test_change_password_ldap_backend(self) -> None:
        ...
        with self.settings(
            ...
            self.assert_json_error(result, "Your Doer password is managed in LDAP")
        ...
        with (
            ...
            self.assertEqual(
                debug_log.output,
                [
                    "DEBUG:doer.ldap:DoerLDAPAuthBackend: Email hamlet@zulip.com does not match LDAP domain example.com."
                ],
            )
...
class UserChangesTest(DoerTestCase):
...
class UserDraftSettingsTests(DoerTestCase):
    ...
    def test_disable_drafts_syncing(self) -> None:
        ...
        draft_dicts = [
            ...
            },
            {
                ...
                "to": [],
                "topic": "",
                "content": "What if made it possible to sync drafts in Doer?",
