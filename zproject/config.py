...
from scripts.lib.doer_tools import get_config as get_config_from_file


class DoerSettingsError(Exception):
    pass
...
# Whether this instance of Doer is running in a production environment.
PRODUCTION = config_file.has_option("machine", "deploy_type")
DEVELOPMENT = not PRODUCTION
secrets_file = configparser.RawConfigParser()
if PRODUCTION:  # nocoverage
    secrets_file.read("/etc/zulip/doer-secrets.conf")
else:
...
def get_mandatory_secret(key: str) -> str:
    ...
    if secret is None:
        if os.environ.get("DISABLE_MANDATORY_SECRET_CHECK") == "True":
            return ""
        raise DoerSettingsError(f'Mandatory secret "{key}" is not set')
    return secret
